...
以下游戏都在真机上跑通过工具箱的功能验收（逐版本验收记录见
[docs/ACCEPTANCE.md](docs/ACCEPTANCE.md)）。

**MV / MZ（原生 NW.js）**

- 再刷一把、再刷一把2：金色传说（MZ）——含坏 global 档启动崩溃修复、
  4700 条技能目录全量读取
- 大千世界2（MZ）
- 三国真龙传（MV，NW.js 0.29 x86 老运行时）
- 三国志潜龙在渊、梦魇：无归、刷啊刷、绿头

**特殊打包的 MV / MZ**

- 魔物召唤森林（Tauri / WebView2 壳，MZ 1.8.0）
- 重装机兵：黎明（EVB 单文件壳，解包后按 Tauri 壳接入）
- 重装归途 0.1.33（Tauri 壳，MV 1.6.1）——已适配引擎重命名、角色背包 / 仓库 /
  独立装备与浏览器存档，并实测修改器、战斗、地图与事件工具。浏览器存档的
  备份 / 恢复需要保持游戏连接；基础属性加点接口无效；独立弹仓不按普通物品修改，
  独立装备不支持数量锁；新增装备请选择基础装备目录，删除时请选择当前物品中的
  具体实例
- 末日风暴 1.9.6（nwjc 字节码 + FT 重命名引擎，MV 1.6.1）——已接入原生金币、
  经验接口，并针对高刷新率屏幕自动限制为 60 FPS，保留游戏作者设定的逻辑速度。
  不支持工具箱的额外加速选项，开启时会明确提示；原生结算文字仍显示基础奖励，
  倍率已作用于实际入账
- 停不下来的轮回（sealed 启动器，MZ）——存档为自定义加密，「运行中数据」
  树编辑器无法离线解析（槽位列表 / 备份正常）
...
- BLACK SOULS Ⅰ / Ⅱ（VX Ace）
- Star Stealing Prince（VX）
- Last Scenario（XP）
- 武界风云传 1.63（RGD 自制 Ruby 引擎 / RGSS103J / v1 加密归档）
- 宝可梦赤途（Pokemon Essentials：存档 / 背包 / 图鉴 / 队员 / 存储箱 /
  新建宝可梦全命令面）——2.9GB 单文件壳，首次启动要先解包，之后游戏本体还要
  加载约 1–2 分钟，这段窗口是黑的；桥接在游戏画出第一帧之前就接上了，工具箱
  会先显示「游戏启动中 / 已连接 · 标题/菜单」，属正常，不是启动失败。存档写
  在游戏目录旁的 `save` 目录，重新解包不会带走

**明确不支持**：重装机兵-宿敌（NB 加密壳，Themida 级防护——任何启动参数、
进程链、文件改动、DLL 注入都会被检测并杀掉游戏，工具箱会识别并明确拒绝，
而不是硬注把游戏搞死）。
...
2. 选中游戏点**启动并注入**，工具会把游戏跑起来并连上修改功能。
   ...
   带保护壳的游戏（一收到启动参数就自杀的那类）没有「启动并注入」按钮：
   双击游戏自己的程序启动，进入游戏后点「附加到运行中」即可。
   有多条可用路线的游戏，卡片上会多出一个路线下拉：默认「自动」由工具箱判定，
   首选路线失败时会在桥接握手前自动换下一条。手动选项用大白话标注——
   「不动原目录」（影子目录）、「原版直接启动」（扩展）——
   悬停可以看到背后的机制和这条路线的判定原因；只有一条安全路线的游戏不显示下拉。
