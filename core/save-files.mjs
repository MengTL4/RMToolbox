...
function basename(value, label = "file name") {
  ...
  if (!name || name === "." || name.includes("..") || /[\\/:\x00-\x1f]/.test(name) || path.basename(name) !== name) {
...
function stat(file, options) {
  try { return fs.statSync(file, options); } catch (error) {
...
function sameFile(a, b) {
  ...
  const left = stat(a, { bigint: true }), right = stat(b, { bigint: true });
  return !!(left && right && left.ino && left.dev === right.dev && left.ino === right.ino);
}

function sameDirectory(a, b) {
  return fs.existsSync(a) && fs.existsSync(b) && fs.realpathSync(a).toLowerCase() === fs.realpathSync(b).toLowerCase();
}

function copy(source, target, preserveTime = true) {
  if (fs.existsSync(target) && fs.lstatSync(target).isSymbolicLink()) throw new Error(`refusing to overwrite a file link: ${target}`);
...
function validLocation(value, gameRoot) {
  if (!value || typeof value.gameRoot !== "string" || typeof value.saveDir !== "string") return null;
  if (gameRoot && path.resolve(value.gameRoot).toLowerCase() !== path.resolve(gameRoot).toLowerCase()) return null;
  const rel = relativeInside(value.gameRoot, value.saveDir);
  if (rel === null) return null;
  const files = Array.isArray(value.files) ? value.files.filter(name => {
    try { return basename(name) === name && SAVE_EXT.test(name); } catch (_) { return false; }
  }) : [];
  return { gameRoot: path.resolve(value.gameRoot), saveDir: path.resolve(value.saveDir), files, rel };
}

function readLocation(shadowRoot, gameRoot) {
  try { return validLocation(JSON.parse(fs.readFileSync(path.join(shadowRoot, LOCATION_FILE), "utf8")), gameRoot); }
  catch (_) { return null; }
}
...
export function reconcileRgssSaves({ gameRoot, shadowRoot, saveLocation, removeSynced = false }) {
  const result = { copied: 0, skipped: 0, conflicts: [], errors: [] };
  const location = validLocation(saveLocation, gameRoot) || readLocation(shadowRoot, gameRoot);
  const directories = new Map([["", new Set()]]);
  if (location) directories.set(location.rel, new Set(location.files.map(name => name.toLowerCase())));
  ...
  if (fs.existsSync(shadowRoot)) {
    for (const name of fs.readdirSync(shadowRoot)) {
      ...
      if (info.isDirectory() && !info.isSymbolicLink() && !directories.has(name)) directories.set(name, new Set());
    }
  }
  for (const [rel, names] of directories) {
    if (!independentDirectory(shadowRoot, rel)) continue;
    const sourceDir = path.join(shadowRoot, rel), targetDir = path.join(gameRoot, rel);
    if (!fs.existsSync(sourceDir) || sameDirectory(sourceDir, targetDir)) continue;
    for (const name of fs.readdirSync(sourceDir)) {
      if (!VANILLA.test(name) && !names.has(name.toLowerCase())) continue;
      const source = path.join(sourceDir, name), target = path.join(targetDir, name);
      try {
        const info = fs.lstatSync(source);
        if (!info.isFile() || info.isSymbolicLink()) continue;
        const previous = stat(target);
        if (previous && !previous.isFile()) throw new Error("destination is not a file");
        if (!previous || info.mtimeMs > previous.mtimeMs) {
          copy(source, target);
          result.copied += 1;
        } else {
          if (info.mtimeMs === previous.mtimeMs && !sameFile(source, target) && !fs.readFileSync(source).equals(fs.readFileSync(target))) {
            result.conflicts.push({ source, target, reason: "equal timestamp, different contents; real file retained" });
          }
          result.skipped += 1;
        }
...
export function createSaveFiles({ projectRoot, resolveLocation }) {
  function backupRoot(gameKey) { return path.join(projectRoot, "backups", basename(gameKey, "game key")); }
  function locationFor(gameKey) {
    const resolved = resolveLocation(gameKey);
    const saveDir = typeof resolved === "string" ? resolved : resolved && resolved.saveDir;
    if (!saveDir || !fs.existsSync(saveDir)) throw new Error(`save directory not found for "${gameKey}"`);
    const shadowRoot = path.join(projectRoot, "runtime", "rgss-shadow", basename(gameKey, "game key"));
    const known = validLocation(resolved && resolved.saveLocation, resolved && resolved.gameRoot) || readLocation(shadowRoot, resolved && resolved.gameRoot);
    ...
    let shadowDir = null;
    if (known && fs.existsSync(shadowRoot) && path.resolve(saveDir).toLowerCase() === known.saveDir.toLowerCase()) {
      if (independentDirectory(shadowRoot, known.rel)) shadowDir = path.join(shadowRoot, known.rel);
    } else if (!known && !(resolved && resolved.saveLocation) && resolved && resolved.gameRoot && fs.existsSync(shadowRoot)) {
      const rel = relativeInside(resolved.gameRoot, saveDir);
      if (rel !== null && independentDirectory(shadowRoot, rel)) shadowDir = path.join(shadowRoot, rel);
    }
  ...
  function shadowFile(location, name) {
    ...
    if (!VANILLA.test(name) && !(location.known && location.known.files.some(file => file.toLowerCase() === name.toLowerCase()))) return null;
  ...
  function backupDir(gameKey, name) {
    ...
    if (!fs.lstatSync(dir).isDirectory() || fs.lstatSync(dir).isSymbolicLink()) throw new Error(`bad backup directory: ${dir}`);
    return dir;
  }
  function saveNames(dir) {
    return fs.readdirSync(dir).filter(name => SAVE_EXT.test(name) && fs.lstatSync(path.join(dir, name)).isFile());
  }
  return {
    backup(gameKey) {
      const { saveDir } = locationFor(gameKey);
      const stamp = new Date().toISOString().replace(/[:.]/g, "-").replace("T", "_").slice(0, 19);
      let destDir = path.join(backupRoot(gameKey), stamp), suffix = 1;
      while (fs.existsSync(destDir)) destDir = path.join(backupRoot(gameKey), `${stamp}-${suffix++}`);
      fs.mkdirSync(destDir, { recursive: true });
      const files = saveNames(saveDir);
      for (const name of files) copy(path.join(saveDir, name), path.join(destDir, name));
    ...
    listBackups(gameKey) {
      ...
      return fs.readdirSync(root).filter(name => {
        ...
        const dir = path.join(root, name), files = saveNames(dir);
        return { name, dir, files: files.length, bytes: files.reduce((sum, file) => sum + fs.statSync(path.join(dir, file)).size, 0), ts: fs.statSync(dir).mtime.toISOString() };
      }).sort((a, b) => b.name.localeCompare(a.name));
    },
    restore(gameKey, name) {
      const sourceDir = backupDir(gameKey, name), location = locationFor(gameKey);
    ...
    deleteSave(gameKey, fileName) {
      ...
      const location = locationFor(gameKey), target = path.join(location.saveDir, name);
