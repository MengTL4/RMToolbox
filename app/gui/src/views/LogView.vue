...
export default {
  ...
  setup: function () {
    ...
    function loadBridgeLog() {
      ...
      bridgeLog.value =
        store.readBridgeLog(bridgeGame.value) || "(no bridge log yet)";
    }
...
<template>
  <n-flex vertical :size="14">
    ...
    <n-card
      size="small"
      title="游戏内桥接日志（runtime/bridge-state/…/bridge.log）"
    >
      <template #header-extra><rm-icon name="log" /></template>
      <n-flex vertical :size="10">
        <n-flex align="center" :size="10" :wrap="true">
          <n-select
            v-model:value="bridgeGame"
            :options="gameOptions"
            placeholder="选择运行中的游戏"
            clearable
            size="small"
            style="width: 260px"
          />
