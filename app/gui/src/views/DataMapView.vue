...
export default {
  ...
  setup: function () {
    ...
    return {
      ...
      mapMark: function (row) {
        return row.id === currentMapId.value ? "当前" : "";
      },
      queryOf: computed(function () {
        return trainer.mapQuery;
      })
    };
  }
};
    ...
    <n-card class="rm-md-list" size="small" title="地图列表">
      <template #header-extra>
        <n-button
          size="tiny"
          quaternary
          :loading="trainer.loading.maps"
          @click="store.loadMaps()"
        >
          <template #icon><rm-icon name="refresh" :size="14" /></template>
        </n-button>
      </template>
      <rm-entry-list
        :entries="trainer.maps"
        :selected-id="trainer.transfer.mapId"
        :query="queryOf"
        value-label=""
        :value-of="mapMark"
        :height="listHeight"
        ...
        @update:query="(v) => (trainer.mapQuery = v)"
        @select="select"
      />
    </n-card>
      ...
      <n-flex vertical :size="14">
        <n-card size="small" title="传送">
          ...
          <n-flex vertical :size="10">
            <n-flex :size="6" :wrap="false">
              <n-input-number
                v-model:value="trainer.transfer.mapId"
                size="small"
                placeholder="地图 ID"
                :min="1"
                :show-button="false"
              ...
              />
              <n-input-number
                v-model:value="trainer.transfer.x"
                size="small"
                placeholder="X"
                :min="0"
                :show-button="false"
              ...
              />
              <n-input-number
                v-model:value="trainer.transfer.y"
                size="small"
                placeholder="Y"
                :min="0"
                :show-button="false"
            ...
            <n-flex :size="8" :wrap="true">
              ...
              <n-button size="small" tertiary @click="here"
                >读取当前坐标</n-button
              >
              <n-button
                size="small"
                tertiary
                @click="
                  trainer.transfer.x = 0;
                  trainer.transfer.y = 0;
                  transfer();
                "
                >传送至 0,0</n-button
              >
              <n-text depth="3" style="font-size: 12px"
                >当前 map
                {{ currentMapId == null ? "-" : currentMapId }}</n-text
              >
            </n-flex>
          </n-flex>
        </n-card>
        ...
        <n-card size="small" title="独立开关">
          ...
          <n-flex vertical :size="10">
            <n-input-group>
              <n-input-number
                v-model:value="data.selfSwitches.mapId"
                size="small"
                placeholder="地图 ID"
                :min="1"
                :show-button="false"
              ...
              >
                <template #icon><rm-icon name="search" :size="15" /></template
                >列出
              </n-button>
              <n-button
                size="small"
                ...
                @click="store.loadSelfSwitches(currentMapId)"
                >当前地图</n-button
              >
            </n-input-group>
            <n-text depth="3" style="font-size: 12px">
              只有被事件设置过的独立开关才存在（RPG Maker
              是稀疏存储），点开关直接切换。
              {{
                data.selfSwitches.entries.length
                  ? "共 " + data.selfSwitches.entries.length + " 个"
                  : ""
              }}
            </n-text>
            <n-data-table
              v-if="data.selfSwitches.entries.length"
              :columns="selfSwitchColumns"
              :data="data.selfSwitches.entries"
              size="small"
              :bordered="false"
              :row-key="(row) => row.eventId + row.letter"
              :max-height="260"
              :loading="data.loading.selfSwitches"
            />
          </n-flex>
