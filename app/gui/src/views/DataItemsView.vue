...
import Component_RmGameIcon from "../parts/RmGameIcon.vue";
import Component_Delta from "../components/Delta.vue";
...
export default {
  ...
  components: {
    ...
    RmDelta: Component_Delta,
    RmGameIcon: Component_RmGameIcon
  },
  props: {
    kind: { type: String, required: true }
  },
  setup: function (props) {
    var draft = store.useDraft(
      function () {
        return props.kind + "." + selectedId.value + ".count";
      },
      function () {
        return selected.value ? count.value : null;
      }
    );
    var lockDraft = store.useDraft(
      function () {
        return props.kind + "." + selectedId.value + ".lock";
      },
      function () {
        return selected.value && locked.value
          ? Number(store.lockedValue(props.kind, selectedId.value))
          : count.value;
      }
    );
    ...
    var entries = computed(function () {
      ...
      return catalog.concat(
        owned
          ...
          .map(function (entry) {
            var base = catalog.find(function (row) {
              return row.id === entry.baseItemId;
            });
    ...
    var selectedId = computed(function () {
      return data.selected[props.kind];
    });
    ...
    var selected = computed(function () {
      ...
      return (
        entries.value.filter(function (entry) {
          return entry.id === id;
        })[0] || null
      );
    });
    ...
    var locked = computed(function () {
      return selected.value
        ? store.isLocked(props.kind, selected.value.id)
        : false;
    });
    ...
    function commit(value) {
      ...
      store
        .setItemCount(props.kind, selected.value.id, Number(value) || 0)
    ...
    function toggleLock(row, enabled) {
      ...
      store.setLock(props.kind, id, value, enabled).then(function (payload) {
        if (!payload) return;
        store.info(
          enabled ? "已锁定 #" + id + " = " + payload.value : "已解锁 #" + id
        );
      });
    }
    ...
    function applyLockValue(value) {
      if (!selected.value) return;
      store
        .setLock(props.kind, selected.value.id, Number(value) || 0, true)
        .then(function (payload) {
    ...
    return {
      ...
      countOf: function (row) {
        return "× " + store.countOf(props.kind, row.id);
      },
      isLocked: function (row) {
        return store.isLocked(props.kind, row.id);
      },
      queryOf: computed(function () {
        return data.query[props.kind];
      })
    };
  }
};
      ...
      <template #header-extra>
        <n-flex align="center" :size="8">
          <n-text depth="3" style="font-size: 12px"
            >已锁定 {{ lockCount }}</n-text
          >
          <n-button
            size="tiny"
            quaternary
            :loading="data.loading.catalog"
            @click="
              store.loadCatalog(kind);
              store.loadCounts();
            "
          >
      ...
      <n-flex v-else vertical :size="16">
        <n-descriptions :column="2" size="small" bordered label-placement="top">
          ...
          <n-descriptions-item label="图标">
            <n-flex align="center" :size="8">
              <rm-game-icon
                v-if="selected.iconIndex != null || selected.iconName"
                :game-key="store.trainer.gameKey"
                :index="selected.iconIndex"
                :icon-name="selected.iconName"
                :size="24"
              />
              <span>{{
                selected.iconIndex == null
                  ? selected.iconName || "-"
                  : selected.iconIndex
              }}</span>
            </n-flex>
          </n-descriptions-item>
          <n-descriptions-item label="当前持有">{{
            count
          }}</n-descriptions-item>
          <n-descriptions-item label="锁定">
            ...
            >
              {{
                locked
                  ? "已锁定 " + store.lockedValue(kind, selected.id)
                  : "未锁定"
              }}
            </n-tag>
          </n-descriptions-item>
        ...
        <div>
          <n-flex align="center" :size="10">
            <n-checkbox
              :checked="locked"
              @update:checked="(v) => toggleLock(selected, v)"
              >锁定这个数量</n-checkbox
            >
            <n-text depth="3" style="font-size: 12px"
              >游戏每帧都会被改回锁定值</n-text
          ...
          <div v-if="locked" style="margin-top: 8px">
            <rm-delta
              v-model:value="lockDraft"
              :min="0"
              placeholder="锁定值"
              @commit="applyLockValue"
            />
          </div>
        </div>
      </n-flex>
