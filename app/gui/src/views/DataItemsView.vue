...
export default {
    ...
    setup: function (props) {
      var draft = store.useDraft(function () { return props.kind + "." + selectedId.value + ".count"; }, function () { return selected.value ? count.value : null; });
      var lockDraft = store.useDraft(function () { return props.kind + "." + selectedId.value + ".lock"; }, function () {
    ...
    <n-flex v-else vertical :size="16">
      <n-descriptions :column="2" size="small" bordered label-placement="top">
        ...
        <n-descriptions-item label="图标">
          <n-flex align="center" :size="8">
            <rm-game-icon v-if="selected.iconIndex != null || selected.iconName"
                          :game-key="store.trainer.gameKey" :index="selected.iconIndex"
                          :icon-name="selected.iconName" :size="24"/>
            <span>{{ selected.iconIndex == null ? (selected.iconName || "-") : selected.iconIndex }}</span>
          </n-flex>
        </n-descriptions-item>
