...
export default {
  ...
  setup: function () {
    function actorField(field) {
      ...
      var identity =
        next && next.identity != null
          ? next.identity + ":" + (session && session.connectedAt)
          : next && next.id;
      return "actor." + (identity == null ? "none" : identity) + "." + field;
    }
    var form = store.useDraft(
      function () {
        return actorField("form");
      },
      function () {
        var next = actor.value || {};
        return {
    ...
    var paramDrafts = store.useDraft(
      function () {
        return actorField("params");
      },
      [0, 0, 0, 0, 0, 0, 0, 0]
    );
    ...
    var actor = computed(function () {
      return trainer.actor;
    });
    var selectedId = computed(function () {
      return data.selected.actor;
    });
    var isEssentials = computed(function () {
      return !!(trainer.live && trainer.live.essentials);
    });

    var inParty = computed(function () {
      if (!actor.value) return false;
      return trainer.party.some(function (member) {
        return member.id === actor.value.id;
      });
    });

    watch(
      function () {
        return trainer.gameKey;
      },
      function () {
        picker.value = null;
    ...
    function toggleParty(row, enabled) {
      store
        .cmdWarn(enabled ? "party.addActor" : "party.removeActor", {
          id: row.id
        })
    ...
    function apply(type, args, label) {
      ...
      return store
        .cmdWarn(type, Object.assign({ id: actor.value.id }, args))
        .then(function (payload) {
    ...
    function applyName() {
      var id = actor.value.id,
        nickname = form.value.nickname;
      return store
        .cmdWarn("actor.name.set", { id: id, name: form.value.name })
        .then(function (payload) {
          if (!payload) return null;
          store.applyActor(payload);
          return store.cmdWarn("actor.nickname.set", {
            id: id,
            nickname: nickname
          });
        })
    ...
    function applyLevel() {
      ...
      var id = actor.value.id,
        amount = Number(form.value.exp) || 0;
      return store
        .cmdWarn("actor.level.set", { id: id, level: form.value.level })
        .then(function (payload) {
          if (!payload) return null;
          store.applyActor(payload);
          return amount > 0
            ? store.cmdWarn("actor.exp.add", { id: id, amount: amount })
            : payload;
        })
        .then(function (payload) {
          if (!payload) return;
    ...
    function bumpParam(index, step) {
      ...
      paramDrafts.value = paramDrafts.value.map(function (v, i) {
        return i === index ? next : v;
      });
    }
    ...
    function openCreate() {
      ...
      store
        .cmd("catalog.query", { kind: "enemy", limit: 20000 })
        .then(function (payload) {
          if (payload) species.value = payload.entries || [];
        })
        .finally(function () {
    ...
    function createSpecies(entry) {
      ...
      store
        ...
        .then(function (payload) {
          ...
          store.ok(
            (entry.name || entry.id) +
              (payload.where === "party" ? " 已入队" : " 已发送到存储箱")
          );
          store.refreshParty();
          store.loadRoster();
    ...
    var visibleEntries = computed(function () {
      ...
      ownedIds.value.forEach(function (id) {
        owned[id] = true;
      });
      return pickerEntries.value.filter(function (entry) {
    ...
    var rosterEntries = computed(function () {
      ...
      trainer.party.forEach(function (member) {
        inPartyIds[member.id] = true;
      });
      return trainer.roster.map(function (entry) {
        return {
          id: entry.id,
          name: entry.name,
          inParty: !!inPartyIds[entry.id],
          box: entry.box || 0
        };
      });
    });
    ...
    return {
      ...
      isInParty: function (row) {
        return row.inParty;
      },
      partyMark: function (row) {
        return row.inParty ? "在队" : row.box ? "盒子 " + row.box : "";
      },
      queryOf: computed(function () {
        return data.query.actor;
      })
    };
  }
};
...
<template>
  <div class="rm-md">
      ...
      <template #header-extra>
        <n-flex align="center" :size="8">
          <n-text depth="3" style="font-size: 12px"
            >在队 {{ trainer.party.length }}</n-text
          >
          <n-button
            v-if="isEssentials"
            size="tiny"
          ...
          <n-button
            size="tiny"
            quaternary
            :loading="trainer.loading.roster"
            @click="
              store.loadRoster();
              store.refreshParty();
            "
    ...
    >
      <template #header-extra>
        <n-flex v-if="actor" align="center" :size="6" :wrap="false">
          ...
          <n-button
            ...
            size="tiny"
            type="primary"
            @click="toggleParty({ id: actor.id, name: actor.name }, true)"
            >入队</n-button
          >
          <n-button
            v-else
            ...
            type="error"
            secondary
            @click="toggleParty({ id: actor.id, name: actor.name }, false)"
            >离队</n-button
          >
          <n-tag
            size="small"
            :bordered="false"
            :type="inParty ? 'success' : 'default'"
          >
            {{ inParty ? "在队" : "未入队" }}
          </n-tag>
        </n-flex>
      ...
      <n-empty
        v-if="!actor"
        description="在左边点一个角色"
        style="padding: 40px 0"
      />
      <n-flex v-else vertical :size="14">
        <n-descriptions :column="3" size="small" bordered label-placement="top">
          <n-descriptions-item label="职业">{{
            actor.className || actor.classId || "-"
          }}</n-descriptions-item>
          <n-descriptions-item label="等级"
            >{{ actor.level }} /
            {{
              actor.maxLevel == null ? "?" : actor.maxLevel
            }}</n-descriptions-item
          >
          <n-descriptions-item label="经验"
            >{{ actor.exp == null ? "-" : actor.exp
            }}{{
              actor.nextLevelExp == null ? "" : " / " + actor.nextLevelExp
            }}</n-descriptions-item
          >
          <n-descriptions-item label="HP"
            >{{ actor.hp }} / {{ actor.mhp }}</n-descriptions-item
          >
          <n-descriptions-item label="MP"
            >{{ actor.mp }} / {{ actor.mmp }}</n-descriptions-item
          >
          <n-descriptions-item label="TP"
            >{{ actor.tp == null ? "-" : actor.tp
            }}{{
              actor.maxTp == null ? "" : " / " + actor.maxTp
            }}</n-descriptions-item
          >
        </n-descriptions>
          ...
          <n-flex vertical :size="12">
            ...
            <n-form-item label="等级 / 追加经验" @keyup.enter="applyLevel">
              <n-flex :size="6" :wrap="false" style="width: 100%">
                ...
                />
                <n-input-number
                  v-model:value="form.exp"
                  :min="0"
                  :show-button="false"
                  style="flex: 1"
                  placeholder="经验 +"
                />
                <n-button type="primary" @click="applyLevel"> 应用 </n-button>
              </n-flex>
            </n-form-item>
            <n-form-item
              label="HP / MP / TP"
              @keyup.enter="
                apply(
                  'actor.vitals.set',
                  { hp: form.hp, mp: form.mp, tp: form.tp },
                  'HP/MP/TP'
                )
              "
            >
              <n-flex :size="6" :wrap="false" style="width: 100%">
                <n-input-number
                  v-model:value="form.hp"
                  :show-button="false"
                  style="flex: 1"
                />
                <n-input-number
                  v-model:value="form.mp"
                  :show-button="false"
                  style="flex: 1"
                />
                <n-input-number
                  v-model:value="form.tp"
                  :show-button="false"
                  style="flex: 1"
                />
                <n-button
        ...
        <div>
          ...
          <div class="rm-params">
            ...
            >
              ...
              <span class="rm-param-cur">{{
                actor.params ? actor.params[index] : "-"
              }}</span>
              <n-input-number
                :value="paramDrafts[index]"
                size="tiny"
                :show-button="false"
                style="width: 72px"
                @update:value="
                  (v) =>
                    (paramDrafts = paramDrafts.map((x, i) =>
                      i === index ? v : x
                    ))
                "
              />
              ...
              <n-button size="tiny" type="primary" @click="applyParam(index)"
                >应用</n-button
              >
            </div>
          </div>
    ...
    <rm-picker
      ...
      "
      @toggle="togglePickerEntry"
      @select-owned="ownedOnly = !ownedOnly"
    />
    ...
    >
      <n-flex vertical :size="10">
        <n-flex align="center" :size="10" :wrap="false">
          <n-input
            v-model:value="createQuery"
            size="small"
            placeholder="搜索名称或内部名"
            clearable
            style="flex: 1"
          >
        ...
        <n-text depth="3" style="font-size: 12px"
          >点一个物种即创建：队伍有空位就入队，满了自动进存储箱。{{
            createFiltered.length
          }}
          条</n-text
        >
        <rm-virtual
          :items="createFiltered"
          :item-size="34"
          :height="420"
          key-field="id"
        >
          <template #default="{ item }">
            <div
              ...
              @click="!createBusy && createSpecies(item)"
            >
              <span class="rm-picker-item">
                <span class="rm-picker-name">
                  <n-text
                    depth="3"
                    style="font-variant-numeric: tabular-nums; flex: none"
                    >{{ item.id }}</n-text
                  >
                  <span class="rm-picker-label">{{
                    item.name || "(无名)"
                  }}</span>
                </span>
              </span>
