...
export default {
  ...
  setup: function () {
    var scene = computed(function () {
      ...
      return trainer.live.inBattle
        ? { label: "战斗中", type: "error" }
        : { label: "地图上", type: "success" };
    });
    ...
    var engine = computed(function () {
      return (
        (trainer.live && trainer.live.engine && trainer.live.engine.maker) ||
        null
      );
    });

    var loadingAny = computed(function () {
      return Object.keys(trainer.loading).some(function (key) {
        return trainer.loading[key];
      });
    });
    ...
    function measure() {
      ...
      var fits = Math.floor(
        (node.clientWidth + COLUMN_GAP) / (COLUMN_MIN + COLUMN_GAP)
      );
  ...
  <n-flex vertical :size="14">
    <n-card size="small">
      <n-flex align="center" :size="10" :wrap="true">
        <strong>游戏概览</strong>
        <n-tag v-if="engine" size="small" :bordered="false" type="info">{{
          engine
        }}</n-tag>
        <n-tag v-if="scene" size="small" :bordered="false" :type="scene.type">{{
          scene.label
        }}</n-tag>
        <n-tag v-if="mapId !== null" size="small" :bordered="false"
          >map {{ mapId }}</n-tag
        >
        <n-tag
          v-if="trainer.gold !== null"
          size="small"
          :bordered="false"
          type="warning"
        >
          <template #icon><rm-icon name="coins" :size="13" /></template
          >{{ trainer.gold }}
        </n-tag>
        ...
        <n-button
          size="small"
          tertiary
          :disabled="!trainer.gameKey"
          @click="$emit('open-data')"
        >
          <template #icon><rm-icon name="layers" :size="15" /></template>物品 /
          角色 / 存档数据 →
        </n-button>
        <n-text
          v-if="trainer.gameKey && !trainer.live"
          depth="3"
