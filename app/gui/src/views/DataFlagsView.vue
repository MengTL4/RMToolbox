...
export default {
  ...
  setup: function (props) {
    var draft = store.useDraft(
      function () {
        return props.kind + "." + selectedId.value + ".value";
      },
      function () {
        return selected.value ? selected.value.value : null;
      }
    );
    var lockDraft = store.useDraft(
      ...
      function () {
        return selected.value
          ? locked.value
            ? store.lockedValue(props.kind, selectedId.value)
            : selected.value.value
          : null;
      }
    );

    var isSwitch = computed(function () {
      return props.kind === "switch";
    });
    var label = computed(function () {
      return isSwitch.value ? "开关" : "变量";
    });
    var entries = computed(function () {
      return data.flags[props.kind] || [];
    });
    ...
    function commitValue(value) {
      ...
      store
        .setFlag(props.kind, selected.value.id, next)
    ...
    function toggleLock(row, enabled) {
      store
        .setLock(props.kind, row.id, enabled ? row.value : null, enabled)
        .then(function (payload) {
          if (!payload) return;
          store.info(
            enabled
              ? "已锁定 " + label.value + " #" + row.id
              : "已解锁 #" + row.id
          );
        });
    }
    ...
    function applyLockValue(value) {
      ...
      var next = isSwitch.value ? !!value : parseValue(value);
      store
        .setLock(props.kind, selected.value.id, next, true)
        .then(function (payload) {
      ...
      <template #header-extra>
        <n-flex align="center" :size="8">
          ...
          <n-button
            size="tiny"
            quaternary
            :loading="data.loading.flags"
            @click="store.loadFlags(kind)"
          >
        ...
        <div v-if="isSwitch">
          <n-flex align="center" :size="12">
            <n-switch
              ...
              @update:value="(v) => setFlag(kind, selected.id, v)"
            />
          ...
          <div style="margin-top: 8px">
            <n-input-group>
              <n-input
                ...
                @keyup.enter="commitValue(draft)"
              />
              <n-button size="small" type="primary" @click="commitValue(draft)"
                >应用</n-button
              >
            </n-input-group>
          </div>
        ...
        <div>
          ...
          <div v-if="locked" style="margin-top: 8px">
            <n-flex v-if="isSwitch" align="center" :size="12">
              <n-switch
                :value="!!lockDraft"
                @update:value="(v) => applyLockValue(v)"
              />
              <n-text depth="3" style="font-size: 12px"
                >锁定为 {{ lockDraft ? "ON" : "OFF" }}</n-text
              >
            </n-flex>
            <n-input-group v-else>
              <n-input
                :value="String(lockDraft == null ? '' : lockDraft)"
                size="small"
                @update:value="(v) => (lockDraft = v)"
                @keyup.enter="applyLockValue(lockDraft)"
              />
              <n-button
                size="small"
                type="warning"
                @click="applyLockValue(lockDraft)"
                >锁定为此值</n-button
              >
            </n-input-group>
          </div>
