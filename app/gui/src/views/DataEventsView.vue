...
export default {
  ...
  setup: function () {
    ...
    const tools = useEventTools("event"),
      showReader = ref(false),
      dialog = store.gameDialog(naive.useDialog());
    const source = computed(() =>
      selected.value ? { kind: "common", id: selected.value.id } : null
    );
    ...
    async function run() {
      ...
      const id = selected.value.id,
        epoch = store.state.selectionEpoch;
      running.value = true;
      try {
        await tools.read({ kind: "common", id });
        if (epoch !== store.state.selectionEpoch || selected.value?.id !== id)
          return;
        if (!tools.state.reader?.execution?.whole) {
          store.warn(
            tools.state.readError ||
              tools.state.reader?.executionError ||
              "执行接口不可用"
          );
          return;
        }
        await confirmExecution(
          tools,
          tools.state.reader.execution.whole,
          true,
          dialog,
          store
        );
    ...
    function toggleReader() {
      ...
      if (showReader.value)
        Vue.nextTick(() =>
          document
            .getElementById("rm-common-event-detail")
    ...
    return {
      ...
      showReader,
      source,
      execution: tools.currentExecution,
      executionActive,
      executionLabels,
      toggleReader,
      select: function (row) {
        data.selected.event = row.id;
      },
  ...
  <div class="rm-md">
    <n-card class="rm-md-list" size="small" title="公共事件">
      ...
      <rm-entry-list
        ...
        :loading="data.loading.events"
        empty-text="等待游戏数据加载…"
        @update:query="(v) => (data.query.event = v)"
        @select="select"
      />
    </n-card>
    <n-card
      id="rm-common-event-detail"
      class="rm-md-detail"
      size="small"
      :title="
        selected
    ...
    >
      ...
      <n-flex v-else vertical :size="14">
        ...
        <n-flex :size="8">
          ...
          <n-button @click="toggleReader">{{
            showReader ? "收起事件解释器" : "事件解释器"
          }}</n-button>
        </n-flex>
        <p v-if="execution?.id" class="rm-event-hint">
          公共事件 #{{ execution.eventId }}：{{
