...
var SNIPPETS = [
  ...
  {
    label: "全队满血",
    code: "$gameParty.members().forEach(a => a.recoverAll())"
  },
  {
    label: "队伍信息",
    code: "$gameParty.members().map(a => [a.actorId(), a.name(), a.level])"
  },
  {
    label: "当前地图",
    code: "[$gameMap.mapId(), $gamePlayer.x, $gamePlayer.y]"
  },
...
var SNIPPETS_ESS = [
  ...
  {
    label: "当前地图",
    code: "[$game_map.map_id, $game_player.x, $game_player.y]"
  }
];
...
export default {
  ...
  setup: function () {
    var gameKey = Vue.computed(function () {
      return store.trainer.gameKey;
    });
    var code = store.useDraft("console.code", "");
    var output = store.useDraft("console.output", []);
    var running = ref(false);
    var outputRef = ref(null);
    var history = store.useDraft("console.history", []);
    ...
    function push(kind, text) {
      ...
      if (output.value.length > 400)
        output.value.splice(0, output.value.length - 400);
      scrollDown();
    }
    ...
    function run() {
      ...
      store
        .send(gameKey.value, "console.eval", { code: code.value })
        .then(function (payload) {
          if (epoch === store.state.selectionEpoch)
            push("out", JSON.stringify(payload.result, null, 2));
        })
        .catch(function (error) {
          if (epoch === store.state.selectionEpoch) push("err", error.message);
        })
        .finally(function () {
          if (epoch === store.state.selectionEpoch) running.value = false;
        });
    }
    ...
    var isRgss = Vue.computed(function () {
      var s = session.value;
      var maker =
        s && s.state && s.state.engine ? s.state.engine.maker : s && s.engine;
    ...
    var hint = Vue.computed(function () {
      if (isEssentials.value) {
        return (
          "Ruby 环境（$player / $bag / $game_switches…）。开启内置调试菜单：$DEBUG = true" +
          "（暂停菜单多出「调试菜单」，F9 控制台解锁）；直接打开：pbDebugMenu。重开游戏后需重设"
        );
      }
      if (isRgss.value)
        return "Ruby 环境（$game_party / $game_switches / $game_variables 等全局变量）";
  ...
  <n-flex vertical :size="14">
      ...
      <n-flex vertical :size="12">
        ...
        <n-flex :size="8" align="center">
          <n-button
            ...
            @click="run"
          >
            <template #icon><rm-icon name="play" :size="15" /></template>执行
            (Ctrl+Enter)
          </n-button>
    ...
    <n-card size="small" title="输出" :content-style="'padding: 0'">
      <div
        ...
        "
      >
        <n-text v-if="!output.length" depth="3" style="font-size: 12.5px"
          >尚无输出。</n-text
        >
        <div
