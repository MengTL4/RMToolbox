...
export default {
    ...
    setup: function () {
      var gameKey = Vue.computed(function () { return store.trainer.gameKey; });
      ...
      function run() {
        ...
        store.send(gameKey.value, "console.eval", { code: code.value })
          .then(function (payload) {
            if (epoch === store.state.selectionEpoch) push("out", JSON.stringify(payload.result, null, 2));
          })
          .catch(function (error) {
            if (epoch === store.state.selectionEpoch) push("err", error.message);
          })
          .finally(function () { if (epoch === store.state.selectionEpoch) running.value = false; });
      }
      ...
      Vue.watch(function () { return store.state.selectionEpoch; }, function () {
        running.value = false; historyIndex.value = null; historyDraft.value = "";
      }, { flush: "sync" });
