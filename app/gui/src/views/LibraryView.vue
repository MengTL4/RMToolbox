...
function routeLabel(id, plan) {
  ...
  for (var i = 0; i < entries.length; i += 1) {
    if (entries[i].id === id)
      return entries[i].userGoal || entries[i].label || id;
  }
  return id || "不可用";
}
...
function routeHint(id, plan) {
  ...
  for (var i = 0; i < entries.length; i += 1) {
    if (entries[i].id === id)
      return [entries[i].mechanismLabel, entries[i].reason]
        .filter(Boolean)
        .join(" · ");
  }
  return "";
...
export default {
  ...
  components: {
    ...
    InjectionGuide: Component_InjectionGuide
  },
  setup: function () {
    var query = ref("");
    ...
    var filtered = computed(function () {
      ...
      return store.state.games.filter(function (game) {
        return (
          game.title.toLowerCase().indexOf(needle) !== -1 ||
    ...
    var connectedCount = computed(function () {
      return store.state.games.filter(function (game) {
        return !!store.sessionFor(game.gameKey);
      }).length;
    });
    ...
    function unavailable(game) {
      ...
      if (game.engine.id === "RM2K" || game.container === "nb-shell")
        return "当前版本暂不支持连接此游戏";
      return "";
    }
    ...
    function attach(game) {
      ...
      dialog.warning({
        ...
        content:
          "此游戏需要结束现有实例，再由工具箱重新启动。请先在游戏中保存进度。",
        positiveText: "重启并连接",
        negativeText: "取消",
        onPositiveClick: function () {
          return store.attach(game);
        }
      });
    }
    function stop(game) {
      dialog.warning({
        title: "结束「" + game.title + "」",
        content: "游戏将关闭，请确认已保存进度。",
        positiveText: "结束游戏",
        negativeText: "取消",
        onPositiveClick: function () {
          return store.stop(game);
        }
      });
    }
    function retry(game) {
    ...
    function routePlan(game) {
      return (
        state.routePlans[game.gameKey] || { selected: null, candidates: [] }
      );
    }
    function routeChoice(game) {
      var plan = routePlan(game);
      var value = state.routeChoices[game.gameKey] || "auto";
      var allowed = (plan.candidates || []).map(function (entry) {
        return entry.id;
      });
    ...
    function routeOptions(game) {
      ...
      return [
        {
          label: plan.preferred
            ? "自动（" + routeLabel(plan.preferred, plan) + "）"
            : "自动",
          value: "auto"
        }
      ].concat(
    ...
    function removeGame(game) {
      dialog.warning({
        ...
        content:
          "把「" +
          game.title +
          "」从游戏库移除？不会删除游戏文件，随时可以重新添加。",
        positiveText: "移除",
        negativeText: "取消",
        onPositiveClick: function () {
          store.removeManualRoot(game.root);
        }
      });
    }
...
<template>
  <n-flex vertical :size="14">
    <div class="rm-page-heading">
      <div>
        <h1>你的游戏</h1>
        <n-text depth="3">连接游戏，调整数值，继续冒险。</n-text>
      </div>
      <n-text depth="3"
        >{{ state.games.length }} 个游戏 · {{ connectedCount }} 个已连接</n-text
      >
    </div>
    <n-card size="small">
    ...
    <n-alert v-if="!state.games.length" type="default" :bordered="false">
      ...
      游戏库是空的。点「添加游戏目录」选择游戏的安装目录（里面有 Game.exe
      的那一层），可以添加多个。
    </n-alert>
    <n-empty
      ...
      >
        <n-flex vertical :size="10">
          <n-flex align="center" :size="11" :wrap="false">
            <n-avatar
              v-if="state.icons[game.gameKey]"
              :src="state.icons[game.gameKey]"
              :size="36"
              :border-radius="10"
              object-fit="cover"
              style="flex: none"
            ...
            <n-icon-wrapper
              ...
              :size="36"
              :border-radius="10"
              :color="sessionFor(game.gameKey) ? '#22c55e' : '#5b8cff'"
            >
              <rm-icon name="gamepad" :size="20" />
            </n-icon-wrapper>
            <n-flex vertical :size="3" style="flex: 1; min-width: 0">
              <n-ellipsis style="font-weight: 600; font-size: 14.5px">{{
                game.title
              }}</n-ellipsis>
          ...
          <n-flex align="center" :size="8" :wrap="true">
            <n-tooltip v-if="statusFor(game).hint" trigger="hover">
              <template #trigger>
                <n-tag
                  ...
                  :bordered="false"
                  :type="
                    state.busy[game.gameKey] ? 'default' : statusFor(game).type
                  "
                >
                  {{
            ...
            <div style="flex: 1"></div>
            <n-select
              v-if="(routePlan(game).candidates || []).length > 1"
              size="small"
              style="width: 150px"
              :value="routeChoice(game)"
              :options="routeOptions(game)"
            ...
            >
              ...
              {{
                routeHint(
                  routePlan(game).preferred || routePlan(game).selected,
                  routePlan(game)
                ) || "当前游戏只有这一条安全启动路线"
              }}
            </n-tooltip>
            ...
            >
              <template #icon><rm-icon name="sliders" :size="15" /></template
              >打开修改器
            </n-button>
          </n-flex>
          <n-flex align="center" :size="6" :wrap="true">
            <n-button
              ...
              @click="attach(game)"
            >
              <template #icon><rm-icon name="zap" :size="14" /></template
              >{{ takeover(game) ? "接管重启并连接" : "附加到运行中" }}
            </n-button>
            <n-button
              v-if="sessionFor(game.gameKey)"
              size="tiny"
              quaternary
              :disabled="!!state.busy[game.gameKey]"
              @click="stop(game)"
            >
              <template #icon><rm-icon name="stop" :size="14" /></template
              >结束游戏
            </n-button>
            <div style="flex: 1"></div>
          ...
          <n-alert
            v-if="
              state.operations[game.gameKey] &&
              (state.operations[game.gameKey].status !== 'success' ||
                !sessionFor(game.gameKey))
            "
            :type="
              state.operations[game.gameKey].status === 'error'
                ? 'error'
                : 'info'
            "
            :show-icon="false"
            :bordered="false"
            class="rm-operation-status"
            aria-live="polite"
          >
            {{ state.operations[game.gameKey].text }}
            <n-button
              v-if="state.operations[game.gameKey].status === 'error'"
              size="tiny"
              tertiary
              :disabled="!!state.busy[game.gameKey]"
              @click="retry(game)"
