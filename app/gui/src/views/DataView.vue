...
export default {
  ...
  setup: function () {
    ...
    var totalLocks = computed(function () {
      ...
      ["item", "weapon", "armor", "switch", "variable"].forEach(
        function (kind) {
    ...
    function confirmClear() {
      dialog.warning({
        title: "清空全部锁定",
        content:
          "会解除所有物品 / 开关 / 变量 / 金钱的锁定（不影响游戏当前数值）。",
        positiveText: "清空",
        negativeText: "取消",
        onPositiveClick: function () {
          store.clearLocks(null);
        }
      });
    }
  ...
  <n-flex vertical :size="14">
    <n-card size="small">
      <n-flex align="center" :size="10" :wrap="true">
        ...
        <n-tag
          size="small"
          :bordered="false"
          :type="totalLocks ? 'warning' : 'default'"
        >
          <template #icon><rm-icon name="toggle" :size="13" /></template>已锁定
          {{ totalLocks }} 项
        </n-tag>
        <n-popover trigger="click" placement="bottom-start">
          <template #trigger
            ><n-button size="small" tertiary>管理锁定配置</n-button></template
          >
          <n-button-group size="small">
            <n-button
              :disabled="!trainer.gameKey"
              @click="store.saveLockFile()"
            >
              <template #icon><rm-icon name="save" :size="15" /></template
              >保存锁定状态
            </n-button>
            <n-button
              :disabled="!trainer.gameKey || !data.lockFileExists"
              @click="store.loadLockFile()"
            >
    ...
    <n-tabs v-else v-model:value="data.tab" type="line" size="small" animated>
      ...
      >
        <data-items
          v-if="
            tab.key === 'item' || tab.key === 'weapon' || tab.key === 'armor'
          "
          :kind="tab.key"
        />
        <data-flags
          v-else-if="tab.key === 'switch' || tab.key === 'variable'"
          :kind="tab.key"
        />
