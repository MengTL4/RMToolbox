...
export default {
  ...
  setup: function () {
    ...
    var slotDraft = store.useDraft("save.slot", function () {
      var next = 1;
    ...
    function refreshFiles() {
      ...
      loading.value = true;
      store
        .send(gameKey.value, "save.list", {})
        .then(function (payload) {
          if (epoch !== store.state.selectionEpoch) return;
        ...
        .catch(function (error) {
          if (epoch === store.state.selectionEpoch)
            store.fail("读取存档列表失败：" + error.message);
        })
        .finally(function () {
          if (epoch === store.state.selectionEpoch) loading.value = false;
        });
    }
    ...
    function onGameChange(value) {
      ...
      loading.value = false;
      refreshBackups();
      if (value && store.sessionFor(value)) refreshFiles();
    }

    async function backup() {
      if (await store.backupSaves(gameKey.value)) refreshBackups();
    }
    ...
    function restore(row) {
      dialog.warning({
        title: "恢复备份",
        content:
          "把这份备份恢复到游戏？现有同名存档会被替换。浏览器存档需要游戏保持连接。",
        positiveText: "覆盖恢复",
        negativeText: "取消",
        onPositiveClick: async function () {
          if (await store.restoreBackup(gameKey.value, row.name))
            refreshFiles();
        }
      });
    }
    ...
    var slots = computed(function () {
      return files.value
        .map(function (entry) {
          return { slot: slotOfEntry(entry), entry: entry };
        })
        .filter(function (row) {
    ...
    var others = computed(function () {
      return files.value.filter(function (entry) {
        return slotOfEntry(entry) === null;
      });
    });

    var occupied = computed(function () {
      var set = Object.create(null);
      slots.value.forEach(function (row) {
        set[row.slot] = true;
      });
      return set;
    });
    ...
    function saveTo(id) {
      ...
      busySlot.value = id;
      store
        .send(gameKey.value, "save.save", { id: id })
        .then(function (payload) {
          if (epoch !== store.state.selectionEpoch) return;
        ...
        .catch(function (error) {
          if (epoch === store.state.selectionEpoch)
            store.fail("存档失败：" + error.message);
        })
        .finally(function () {
          if (epoch === store.state.selectionEpoch) busySlot.value = 0;
        });
    }
    ...
    function loadSlot(row) {
      dialog.warning({
        ...
        onPositiveClick: function () {
          ...
          store
            .send(gameKey.value, "save.load", { id: row.slot })
            .then(function (payload) {
              if (epoch !== store.state.selectionEpoch) return;
              ...
              if (store.trainer.gameKey !== gameKey.value)
                store.selectGame(gameKey.value);
              store.reloadAfterSceneChange();
            })
            .catch(function (error) {
              if (epoch === store.state.selectionEpoch)
                store.fail("读档失败：" + error.message);
            })
            .finally(function () {
              if (epoch === store.state.selectionEpoch) busySlot.value = 0;
            });
    ...
    function deleteSlot(row) {
      dialog.warning({
        ...
        negativeText: "取消",
        onPositiveClick: async function () {
          if (await store.deleteSaveFile(gameKey.value, row.entry.name))
            refreshFiles();
        }
      });
    }
    ...
    Vue.watch(
      function () {
        return store.state.selectionEpoch;
      },
      function () {
        onGameChange(gameKey.value);
      },
      { immediate: true }
    );
    ...
    var slotColumns = [
      ...
      },
      {
        ...
        width: 90,
        render: function (row) {
          return formatSize(row.entry.size);
        }
      },
      {
        title: "修改时间",
        ...
        width: 172,
        render: function (row) {
          return formatTime(row.entry.mtime);
        }
      },
      {
        title: "",
            ...
            { size: 4, wrap: false, justify: "end" },
            {
              default: function () {
                return [
                  ...
                  actionButton("覆盖", "save", {
                    onClick: function () {
                      overwriteSlot(row);
                    }
                  }),
                  actionButton("删除", "trash", {
                    onClick: function () {
                      deleteSlot(row);
                    }
                  })
    ...
    var slotHeight = computed(function () {
      return Math.max(200, store.viewport.height - 520);
    });
    ...
    return {
      ...
      openSaveDir: function () {
        store.openPath(saveDir.value);
      },
      openBackupDir: function () {
        store.openPath(store.backupsDir(gameKey.value));
      }
    };
  }
};
...
<template>
  <n-flex vertical :size="14">
    <n-card size="small">
      <n-flex align="center" :size="10" :wrap="true">
        ...
        <n-button
          ...
          @click="backup"
        >
          <template #icon><rm-icon name="archive" :size="15" /></template
          >备份存档
        </n-button>
        <n-button
          size="small"
          ...
          @click="openSaveDir"
        >
          <template #icon><rm-icon name="folder" :size="15" /></template
          >打开存档目录
        </n-button>
      </n-flex>
    </n-card>
    <n-result
      ...
      size="small"
      title="还没有选中游戏"
      description="在「游戏库」启动一个游戏，然后在上面的下拉框里选它。"
      style="padding: 40px 0"
    >
      <template #footer>
        <n-button type="primary" @click="$emit('open-library')">
          <template #icon><rm-icon name="gamepad" :size="15" /></template
          >去游戏库
        </n-button>
      </template>
    </n-result>
    <template v-else>
      <n-card size="small" title="存档槽">
        <template #header-extra>
          <n-flex align="center" :size="8">
            ...
            <n-input-number
              v-model:value="slotDraft"
              size="small"
              :min="1"
              :show-button="false"
              style="width: 64px"
              @keyup.enter="saveToDraft"
            />
            <n-button
              size="small"
              type="primary"
        ...
        <n-flex vertical :size="10">
          <n-text v-if="saveDir" depth="3" style="font-size: 12px">{{
            saveDir
          }}</n-text>
          <n-text
            v-else-if="saveStorage === 'webstorage'"
            depth="3"
            style="font-size: 12px"
            >存档保存在游戏的浏览器数据中。备份、恢复和删除需要保持游戏连接。</n-text
          >
          <n-text
            v-else-if="saveStorage === 'native'"
            depth="3"
            style="font-size: 12px"
            >存档由游戏自行管理。备份和恢复需要保持游戏连接；删除是否可用取决于游戏的原生接口。</n-text
          >
          <n-text v-if="localOnly" depth="3" style="font-size: 12px"
            >此处保存和读取本机存档。云端存档请通过游戏内的云存档菜单管理。</n-text
          >
          <n-data-table
      ...
      <n-card v-if="others.length" size="small" title="系统文件">
        <n-flex vertical :size="10">
          <n-text depth="3" style="font-size: 12px"
            >global / config 等游戏自己的文件，只读展示，别手动删。</n-text
          >
          <n-data-table
      ...
      <n-card size="small" title="历史备份">
        <template #header-extra>
          <n-flex align="center" :size="8">
            <n-button
              ...
              @click="openBackupDir"
            >
              <template #icon><rm-icon name="folder" :size="14" /></template
              >打开备份目录
            </n-button>
        ...
        <n-flex vertical :size="10">
          <n-text depth="3" style="font-size: 12px"
            >备份存放在 RMCH/backups/&lt;游戏名&gt;/&lt;时间戳&gt;/</n-text
          >
          <n-data-table
            v-if="backups.length"
          ...
          <n-empty
            ...
            size="small"
            style="padding: 20px 0"
            description="还没有备份 —— 改存档前点一下「备份存档」"
          />
        </n-flex>
      </n-card>
    </template>
