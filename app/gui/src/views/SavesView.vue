...
export default {
    ...
    setup: function () {
      ...
      function restore(row) {
        dialog.warning({
          title: "恢复备份",
          content: "把这份备份恢复到游戏？现有同名存档会被替换。浏览器存档需要游戏保持连接。",
          positiveText: "覆盖恢复",
          negativeText: "取消",
          onPositiveClick: async function () {
            if (await store.restoreBackup(gameKey.value, row.name)) refreshFiles();
          }
        });
      }
      ...
      function saveTo(id) {
        ...
        store.send(gameKey.value, "save.save", { id: id })
          ...
          .catch(function (error) { if (epoch === store.state.selectionEpoch) store.fail("存档失败：" + error.message); })
          .finally(function () { if (epoch === store.state.selectionEpoch) busySlot.value = 0; });
      }
        ...
        dialog.warning({
          ...
          onPositiveClick: function () {
            ...
            store.send(gameKey.value, "save.load", { id: row.slot })
              ...
              .catch(function (error) { if (epoch === store.state.selectionEpoch) store.fail("读档失败：" + error.message); })
              .finally(function () { if (epoch === store.state.selectionEpoch) busySlot.value = 0; });
          }
        });
      }
      ...
      function deleteSlot(row) {
        dialog.warning({
          title: "删除槽 " + row.slot + "？",
          content: "会删除存档 " + row.entry.name + "，不可恢复（备份目录里的备份不受影响）。",
          positiveText: "删除",
          negativeText: "取消",
          onPositiveClick: async function () {
            if (await store.deleteSaveFile(gameKey.value, row.entry.name)) refreshFiles();
          }
        });
      }
      ...
      Vue.watch(function () { return store.state.selectionEpoch; }, function () { onGameChange(gameKey.value); }, { immediate: true });
  ...
  <n-card size="small">
    <n-flex align="center" :size="10" :wrap="true">
      ...
      <n-button size="small" secondary type="primary" :disabled="!gameKey" @click="backup">
        <template #icon><rm-icon name="archive" :size="15"/></template>备份存档
      </n-button>
    ...
    <template #header-extra>
      <n-flex align="center" :size="8">
        ...
        <n-input-number v-model:value="slotDraft" size="small" :min="1" :show-button="false"
                        style="width: 64px" @keyup.enter="saveToDraft"/>
