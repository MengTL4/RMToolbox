...
import Component_RmJsonEditor from "../parts/RmJsonEditor.vue";
...
export default {
  ...
  components: {
    ...
    RmJsonEditor: Component_RmJsonEditor
  },
  setup: function () {
    ...
    var snapshots = Vue.reactive(new Map());
    var source = computed(function () {
      var saved = snapshots.get(store.trainer.gameKey);
      return saved ? saved.json : data.tree.json;
    });
    // Serialize only when leaving a game, before selectGame resets live data.
    Vue.watch(
      function () {
        return store.state.selectionEpoch;
      },
      function () {
        if (store.trainer.gameKey && editor.value) {
          var text = editor.value.getText();
          if (text)
            snapshots.set(store.trainer.gameKey, {
              json: text,
              dirty: dirty.value
            });
        }
    ...
    Vue.watch(
      ...
      function (key) {
        var saved = snapshots.get(key);
        dirty.value = !!(saved && saved.dirty);
      }
    );
    ...
    function refetch() {
      return store.loadSaveTree().then(function (payload) {
        if (payload) {
          ...
          if (editor.value) editor.value.reload(payload.json);
        }
        return payload;
      });
    }
        ...
        onPositiveClick: function () {
          store.applySaveTree(json, reloadMap.value).then(function (payload) {
            if (payload) {
              ...
              if (editor.value) editor.value.setPristine();
            }
          });
        }
      });
    ...
    var treeHeight = computed(function () {
      return Math.max(280, store.viewport.height - 250);
    });
    ...
    return {
      ...
      pull: pull,
      apply: apply,
      onDirty: function (value) {
        var saved = snapshots.get(store.trainer.gameKey);
        dirty.value = value || !!(saved && saved.dirty);
      },
  ...
  <n-card size="small" title="存档数据（运行时）">
    <template #header-extra>
      <n-flex align="center" :size="10">
        ...
        <n-tag v-if="dirty" size="small" :bordered="false" type="warning"
          >有未应用的修改</n-tag
        >
        <n-text v-if="sizeText" depth="3" style="font-size: 12px">{{
          sizeText
        }}</n-text>
    ...
    <n-flex vertical :size="10">
      <n-alert v-if="data.tree.error" type="error" :bordered="false">{{
        data.tree.error
      }}</n-alert>
      <rm-json-editor
        ref="editor"
      ...
      >
        <template #menu>
          <button
            class="rm-je-btn rm-je-btn-primary"
            :disabled="data.tree.loading"
            @click="pull"
          >
          ...
          <button
            class="rm-je-btn"
            :disabled="!source || data.tree.applying"
            @click="apply"
          >
      ...
      <n-collapse>
        <n-collapse-item title="操作说明" name="help">
          <n-text depth="3" style="font-size: 12px">
            右键任意行（或点行首 ▾）= 追加 / 插入 / 复制 / 移除 /
            改类型；拖手柄可移动节点； Ctrl+Z
            撤销。增删键会改变存档结构，与游戏的类不匹配时可能读档才报错 ——
            建议先在「存档」页备份。
          </n-text>
        </n-collapse-item>
      </n-collapse>
