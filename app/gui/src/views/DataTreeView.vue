...
export default {
    name: "DataTreeView",
    components: { RmIcon: Component_RmIcon, RmJsonEditor: Component_RmJsonEditor },
    setup: function () {
      ...
      Vue.watch(function () { return store.state.selectionEpoch; }, function () {
        if (store.trainer.gameKey && editor.value) {
          var text = editor.value.getText();
          if (text) snapshots.set(store.trainer.gameKey, { json: text, dirty: dirty.value });
        }
      }, { flush: "sync" });
      Vue.watch(function () { return store.trainer.gameKey; }, function (key) {
      ...
      function refetch() {
        return store.loadSaveTree().then(function (payload) {
          if (payload) {
            snapshots.set(store.trainer.gameKey, { json: payload.json, dirty: false });
        ...
        dialog.warning({
          ...
          onPositiveClick: function () {
            store.applySaveTree(json, reloadMap.value).then(function (payload) {
              if (payload) {
                snapshots.set(store.trainer.gameKey, { json: json, dirty: false });
