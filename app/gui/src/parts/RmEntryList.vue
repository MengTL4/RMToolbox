...
export default {
  name: "RmEntryList",
  components: { RmIcon: Component_RmIcon, RmGameIcon: Component_RmGameIcon },
  props: {
    ...
    var columns = computed(function () {
      ...
      list.push({
        ...
        render: function (row) {
          ...
          if (row.iconIndex != null ? !icons.ready.value : !row.iconName)
            return label;
          return h("span", { class: "rm-entry-name" }, [
            h(Component_RmGameIcon, {
...
<template>
  <n-flex vertical :size="8">
    ...
    <n-text depth="3" style="font-size: 12px"
      >{{ filtered.length }} / {{ entries.length }} 条</n-text
    >
    <n-data-table
      :columns="columns"
      :data="filtered"
      ...
      :loading="loading"
      :row-class-name="
        (row) => (row.id === selectedId ? 'rm-row-selected' : '')
      "
      :row-props="
        (row) => ({
