...
export default {
    ...
    setup: function (props, ctx) {
      var icons = RMCH.iconset.useGame(function () { return props.gameKey; });
      ...
      var columns = computed(function () {
        ...
        list.push({
          ...
          render: function (row) {
            ...
            if (row.iconIndex != null ? !icons.ready.value : !row.iconName) return label;
            return h("span", { class: "rm-entry-name" }, [
              h(Component_RmGameIcon, { gameKey: props.gameKey, index: row.iconIndex, iconName: row.iconName, size: 20 }),
              label
            ]);
          }
        });
