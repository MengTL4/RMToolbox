...
import Component_RmGameIcon from "./RmGameIcon.vue";
import Component_RmVirtual from "./RmVirtual.vue";
...
export default {
  ...
  setup: function (props) {
    ...
    // The modal stays mounted while hidden; visibility owns its subscription.
    var icons = RMCH.iconset.useGame(function () {
      return props.show ? props.gameKey : "";
    });

    var owned = computed(function () {
      var set = Object.create(null);
      props.ownedIds.forEach(function (id) {
        set[id] = true;
      });
      return set;
    });
    ...
    var filtered = computed(function () {
      ...
      return props.entries.filter(function (entry) {
        return (
          String(entry.name || "")
            .toLowerCase()
            .indexOf(needle) !== -1 || String(entry.id) === needle
        );
      });
    });

    var ownedCount = computed(function () {
      return props.ownedIds.length;
    });
    ...
    <n-flex vertical :size="10">
      <n-flex align="center" :size="10" :wrap="false">
        ...
        <n-button
          ...
          "
        >
          <template #icon><rm-icon name="check" :size="15" /></template
          >只看已拥有 ({{ ownedCount }})
        </n-button>
      </n-flex>
      <n-text depth="3" style="font-size: 12px"
        >勾选 = 拥有，取消勾选 = 移除。{{ filtered.length }} 条</n-text
      >
      <rm-virtual
        :items="filtered"
        :item-size="38"
            ...
            >
              <span class="rm-picker-item">
                <span class="rm-picker-name">
                  <rm-game-icon
                    v-if="item.iconIndex != null ? iconsReady : !!item.iconName"
                    :game-key="gameKey"
                ...
                <n-text v-if="item.note" depth="3" class="rm-picker-note">{{
                  item.note
                }}</n-text>
              </span>
            </n-checkbox>
