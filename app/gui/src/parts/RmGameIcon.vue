...
function describeGame(key) {
    ...
    var game = store.state.games.find(function (entry) { return entry.gameKey === key; });
    ...
    return { key: key, root: root, engine: engine,
      identity: JSON.stringify([key, root, engine, session && session.connectedAt]) };
  }
...
function recordFor(info) {
    ...
    if (!record || record.identity !== info.identity) {
      record = {
        key: info.key, root: info.root, identity: info.identity,
        cell: /^RGSS/i.test(info.engine) ? 24 : 32,
        hasSheet: info.engine !== "RGSS1", users: 0,
        state: Vue.ref("none"), image: null, cols: 0,
        tiles: new Map(), names: new Map(), nameVersion: Vue.ref(0), nameRetry: null,
        promise: null, retry: null, retryAt: 0
      };
...
function fetchSheet(record) {
    ...
    return Promise.resolve().then(function () {
      ...
      return RMCH.store.send(record.key, "assets.iconset", {}).then(function (payload) {
        if (!payload || !payload.dataUrl) throw new Error("no bridge icon sheet");
...
function scheduleRetry(record) {
    if (!record.users || record.retry !== null || games.get(record.key) !== record) return;
    record.retry = setTimeout(function () {
      record.retry = null;
...
function loadSheet(record) {
    if (!record.hasSheet || record.promise || record.state.value === "ready") return;
    if (Date.now() < record.retryAt) { scheduleRetry(record); return; }
    record.state.value = "loading";
    record.promise = fetchSheet(record).then(function (image) {
...
function tile(record, index) {
    ...
    ctx.drawImage(record.image, (id % record.cols) * cell, Math.floor(id / record.cols) * cell,
...
function scheduleNames(record) {
    if (!record.users || record.nameRetry !== null || games.get(record.key) !== record) return;
    var next = Infinity;
    record.names.forEach(function (entry) { if (!entry.url) next = Math.min(next, entry.retryAt); });
    if (!isFinite(next)) return;
    record.nameRetry = setTimeout(function () {
      record.nameRetry = null;
      if (!record.users || games.get(record.key) !== record) return;
      record.names.forEach(function (entry, name) {
        if (!entry.url && entry.retryAt <= Date.now()) readNamedIcon(record, name);
      });
...
function useGame(getKey) {
    ...
    Vue.watch(function () {
      ...
      if (info && current.value && current.value.identity === info.identity) return;
    ...
    Vue.onScopeDispose(function () { release(current.value); current.value = null; });
    return {
      ready: Vue.computed(function () { return !!current.value && current.value.state.value === "ready"; }),
      loading: Vue.computed(function () { return !!current.value && current.value.state.value === "loading"; }),
...
export default {
    ...
    setup: function (props) {
      var icons = useGame(function () { return props.gameKey; });
      var src = Vue.computed(function () { return icons.image(props.index, props.iconName); });
      return function () {
        if (src.value) return Vue.h("img", {
          class: "rm-gicon", src: src.value, width: props.size, height: props.size, draggable: false, alt: ""
        });
        if (props.index != null && icons.loading.value) return Vue.h("span", {
          class: "rm-gicon rm-gicon-ph", style: { width: props.size + "px", height: props.size + "px" }
        });
        return null;
      };
    }
