...
export default {
  ...
  setup: function (props, ctx) {
    ...
    function syncMenu() {
      menuEl.value = host.value
        ? host.value.querySelector(".jsoneditor-menu")
        : null;
    }
      ...
      var opts = {
        ...
        onChange: function () {
          ...
          dirtyTimer = setTimeout(function () {
            ...
            try {
              markDirty(editor.getText() !== lastText);
            } catch (_) {}
          }, 400);
        },
        onModeChange: function () {
          Vue.nextTick(syncMenu);
        },
        onError: function (error) {
          RMCH.store.fail(
            "编辑器错误：" + (error && error.message ? error.message : error)
          );
        }
      };
      if (props.readonly)
        opts.onEditable = function () {
          return false;
        };
      return opts;
    }
