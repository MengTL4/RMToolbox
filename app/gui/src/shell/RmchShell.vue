...
export default {
  ...
  setup: function (props) {
    ...
    store.attachFeedback({
      message: naive.useMessage(),
      dialog: naive.useDialog()
    });
    ...
    var menuOptions = computed(function () {
      ...
      return [
        ...
        entries[4],
        {
          key: "advanced",
          label: "高级工具",
          icon: RMCH.icon("terminal", 18),
          children: [entries[3], entries[5]]
        }
      ];
    });
    ...
    var title = computed(function () {
      var found = TABS.filter(function (entry) {
        return entry.key === tab.value;
      })[0];
    ...
    var serverStatus = computed(function () {
      if (store.state.bootError)
        return { type: "error", dot: "#ef4444", text: "服务异常" };
      if (!store.state.ready)
        return { type: "warning", dot: "#f59e0b", text: "服务启动中…" };
      return { type: "success", dot: "#22c55e", text: "服务正常" };
    });

    var connected = computed(function () {
      return store.state.sessions.filter(function (session) {
    ...
    // A newer release is only ever a link. The toolbox never downloads or
    // replaces itself: it is a 200 MB NW.js app and swapping the running
    // binary underneath the user is not worth the failure modes.
    var update = computed(function () {
      var info = store.state.update;
      if (!info) return null;
      if (info.updateAvailable) {
        return {
          show: true,
          text: "有新版本 v" + info.latest + "（当前 v" + info.current + "）",
          url: info.url
        };
      }
      if (info.ok)
...
<template>
  <n-layout has-sider position="absolute">
    ...
    >
      <div class="rm-brand">
        ...
        <span v-if="!collapsed" class="rm-brand-text">
          <strong style="font-size: 15px; letter-spacing: 0.2px"
            >RM 工具箱</strong
          >
    ...
    <n-layout>
      <n-layout-header bordered class="rm-header">
        ...
        <div class="rm-current-game">
          ...
          <n-select
            :value="store.trainer.gameKey"
            :options="store.currentGameOptions.value"
            placeholder="选择已连接的游戏"
            size="small"
            filterable
            clearable
            @update:value="store.selectGame"
          />
          <n-tooltip v-if="currentStatus.hint" trigger="hover">
            <template #trigger>
              <n-tag
                v-if="store.trainer.gameKey"
                size="small"
                :bordered="false"
          ...
          <n-tag
            v-else-if="store.trainer.gameKey"
            size="small"
            :bordered="false"
            :type="currentStatus.type"
          >
        ...
        <n-tooltip trigger="hover">
          <template #trigger>
            <span class="rm-service-status" :aria-label="serverStatus.text">
              <span
                class="rm-status-dot"
                :style="{ background: serverStatus.dot }"
              ></span>
            </span>
          </template>
          {{ serverStatus.text }} · 本机桥接服务 127.0.0.1:{{
            state.port || 47412
          }}
        </n-tooltip>
        <n-tooltip trigger="hover">
        ...
        <n-tooltip trigger="hover">
          <template #trigger>
            ...
            >
              <template #icon
                ><rm-icon :name="dark ? 'sun' : 'moon'" :size="17"
              /></template>
            </n-button>
          </template>
      ...
      >
        ...
        <n-result
          v-if="
            tab !== 'library' && tab !== 'log' && !store.currentConnected.value
          "
          status="info"
          :title="
            store.trainer.gameKey ? '当前游戏已断开' : '选择一个游戏，开始修改'
          "
          :description="
            store.trainer.gameKey
              ? '已保留当前游戏和编辑草稿。重新连接后可以继续，未提交的修改不会自动应用。'
              : '到游戏库启动或连接游戏，再打开修改器。'
          "
          class="rm-empty-state"
        >
          <template #footer
            ><n-button type="primary" @click="tab = 'library'"
              >前往游戏库</n-button
            ></template
          >
        </n-result>
    ...
    <n-modal v-model:show="showAbout">
      ...
      >
        <div class="rm-about">
          <span class="rm-brand-mark rm-about-mark"
            ><rm-icon name="sliders" :size="22"
          /></span>
          <div style="display: flex; flex-direction: column; gap: 2px">
            <span
              ><strong style="font-size: 16px">RM 工具箱</strong>
              <n-text depth="3" style="font-size: 12px; margin-left: 8px"
                >v{{ about.version }}</n-text
              ></span
            >
            <n-text depth="2" style="font-size: 12.5px"
              >RPG Maker MV/MZ 与 XP/VX/VX Ace 游戏工具</n-text
            >
          </div>
        </div>
        <n-descriptions
        ...
        >
          <n-descriptions-item label="NW.js 运行时">{{
            about.nw
          }}</n-descriptions-item>
          ...
          <n-descriptions-item label="桥接端口"
            >{{ state.port || 47412 }}（仅本机）</n-descriptions-item
          >
          <n-descriptions-item label="项目路径">{{
            about.root
          }}</n-descriptions-item>
          <n-descriptions-item v-if="update" label="版本更新">
            <n-text v-if="update.url" depth="2">{{ update.text }} ——</n-text>
            <n-text v-else depth="2">{{ update.text }}</n-text>
            <n-button
              v-if="update.url"
              text
              type="primary"
              size="tiny"
              @click="store.openExternal(update.url)"
            >
              前往下载
            </n-button>
          </n-descriptions-item>
        ...
        <n-text
          depth="3"
          style="display: block; margin-top: 14px; font-size: 11.5px"
        >
