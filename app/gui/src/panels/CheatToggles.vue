...
export default {
    ...
    setup: function () {
      ...
      function toggle(key, value) {
        ...
        patch[key] = value;
        pending.value = true;
        store.setOptions(patch).then(function (payload) {
          if (payload) store.ok("开关已更新");
        }).finally(function () { pending.value = false; });
      }
      return { trainer: trainer, options: BOOL_OPTIONS, toggle: toggle, pending: pending };
    },
...
<n-card size="small" title="常用开关">
  ...
  <div class="rm-pairs">
    <label v-for="[key, label, tip] in options" :key="key" class="rm-pair">
      ...
      <n-switch size="small" :disabled="pending" :value="!!trainer.options[key]" @update:value="v => toggle(key, v)"/>
    </label>
  </div>
</n-card>
</template>
