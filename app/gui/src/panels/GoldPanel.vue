...
export default {
    ...
    setup: function () {
      ...
      function apply(type) {
        ...
        store.cmdWarn(type, args).then(function (payload) {
          if (!payload) return;
      ...
      return { trainer: trainer, amount: amount, apply: apply, display: display, pending: pending };
    },
...
<n-card size="small" title="金钱">
  ...
  <n-flex vertical :size="12">
    ...
    <n-input-group>
      <n-input-number v-model:value="amount" placeholder="输入金额" :min="0" :show-button="false" style="flex: 1" @keyup.enter="apply('gold.set')"/>
      <n-button type="primary" :loading="pending" :disabled="amount == null" @click="apply('gold.set')">设为此值</n-button>
      <n-button secondary :disabled="pending || amount == null" @click="apply('gold.add')">增加此值</n-button>
    </n-input-group>
    <n-text depth="3" style="font-size: 12px">输入不会立即生效 · Enter 设为此值</n-text>
  </n-flex>
</n-card>
</template>
