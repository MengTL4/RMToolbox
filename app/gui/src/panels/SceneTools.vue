...
var REPAIRS = [
  {
    action: "clearCurrentEvent",
    label: "清除当前事件",
    icon: "close",
    hint: "事件卡住不动时清空解释器"
  },
  {
    action: "clearPictures",
    label: "清除所有图片",
    icon: "trash",
    hint: "残留立绘 / 黑幕挡住画面"
  },
    ...
    var scenes = computed(function () {
      return data.scenes
        .filter(function (name) {
          return !SCENE_SKIP[name];
        })
        .map(function (name) {
          return {
            name: name,
            label: SCENE_LABELS[name] || name.replace(/^Scene_/, "")
          };
        });
    });

    function repair(entry) {
      if (entry.action !== "gotoTitle")
        return store.repair(entry.action, entry.label);
      dialog.warning({
        title: "转至标题",
        ...
        onPositiveClick: function () {
          store.repair(entry.action, entry.label);
        }
      });
    }
      ...
      <div>
        ...
        <n-flex :size="6" :wrap="true">
          <n-button
            ...
            size="small"
            tertiary
            @click="store.pushScene(scene.name)"
            >{{ scene.label }}</n-button
          >
          <n-button size="small" secondary @click="store.popScene()">
            <template #icon><rm-icon name="close" :size="14" /></template
            >弹出场景
          </n-button>
          <n-text v-if="!scenes.length" depth="3" style="font-size: 12px"
            >等待游戏数据…</n-text
          >
        </n-flex>
      </div>
        ...
        <n-flex :size="6" :wrap="true" style="margin-top: 8px">
          ...
          >
            <template #trigger>
              <n-button
                size="small"
                :type="entry.action === 'gotoTitle' ? 'error' : 'default'"
                :secondary="entry.action === 'gotoTitle'"
                :tertiary="entry.action !== 'gotoTitle'"
                @click="repair(entry)"
              >
                <template #icon
                  ><rm-icon :name="entry.icon" :size="14" /></template
                >{{ entry.label }}
              </n-button>
            </template>
            {{ entry.hint }}
