...
async function main() {
  ...
  if (process.argv.includes("--launch")) {
    ...
    console.log(
      `launched pid ${summary.pid} (GUI-simulated), waiting for seed + bridge session...`
    );
  } else {
    await ensureServer({ projectRoot, port: PORT, token: TOKEN });
    if (!(await serverReachable(PORT)))
      throw new Error(`no ws server on ${PORT}`);
  }
  ...
  assertEqual(
    "runtime.info.title",
    info.engine && info.engine.title,
    scan.title
  );
  check(
    "runtime.info.hooks",
    info.hooks && info.hooks.targets && info.hooks.targets.length >= 29,
  ...
  check(
    "player.location",
    loc.mapId !== null && loc.mapId !== undefined,
  ...
  check(
    "item.list.contains",
    inv.entries.some((e) => e.kind === "item" && e.id === 2 && e.count === 3),
  ...
  check(
    "item.set.zero",
    !invAfter.entries.some((e) => e.kind === "item" && e.id === 2),
    ""
  );
  ...
  check(
    "catalog.item.query",
    catalog.total >= 1 && catalog.entries[0].name.includes("怪物"),
    `total=${catalog.total} name=${catalog.entries[0] && catalog.entries[0].name}`
  );
  check(
    "catalog.item.icons",
    catalog.entries.some(
      (e) => e.iconIndex !== null && e.iconIndex !== undefined
    ),
    "iconIndex present"
  );
  ...
  check(
    "actor.exp.add roundtrip",
    expUp >= expFresh + 100 && expBack <= expUp,
  ...
  await send("actor.param.add", { id: actorId, paramId: 0, value: -10 });
  const plusBack = (await send("actor.info", { id: actorId })).actor
    .paramPlus[0];
  check(
    "actor.param.add roundtrip",
    plusUp === 10 && plusBack === 0,
    `up=${plusUp} back=${plusBack}`
  );
  ...
  check(
    "actor.vitals.set",
    vitals.hp === 1 && vitals.mp === 1,
    `hp=${vitals.hp} mp=${vitals.mp}`
  );
  await send("actor.vitals.set", {
    id: actorId,
    hp: party0.members[0].hp,
    mp: party0.members[0].mp
  });
  ...
  check(
    "actor.name.set",
    (await send("actor.info", { id: actorId })).actor.name ===
      party0.members[0].name,
    ""
  );
  await send("actor.nickname.set", {
    id: actorId,
    nickname: party0.members[0].nickname
  });
  check(
    "actor.nickname.set",
    (await send("actor.info", { id: actorId })).actor.nickname ===
      party0.members[0].nickname,
    ""
  );

  // learn a skill the actor does not have, then forget it
  const owned = new Set(party0.members[0].skills.map((s) => s.id));
  const freeSkill = skills.entries
    .map((e) => e.id)
  ...
  const stateId = (states.entries.find((e) => e.id !== 1) || states.entries[0])
    .id;
  ...
  const spareActor = (
    await send("catalog.query", { kind: "actor", limit: 8 })
  ).entries
    .map((e) => e.id)
    .find((id) => !party0.members.some((m) => m.id === id));
  if (spareActor) {
    await send("party.addActor", { id: spareActor });
    const joined = (await send("party.info")).members.some(
      (m) => m.id === spareActor
    );
    await send("party.removeActor", { id: spareActor });
    const left = (await send("party.info")).members.some(
      (m) => m.id === spareActor
    );
    check(
      "party.addActor/removeActor",
      joined && !left,
      `actor=${spareActor} joined=${joined} left=${left}`
    );
  } else {
    check(
      "party.addActor/removeActor",
      true,
      "skipped — no spare actor in catalog"
    );
  }
  ...
  check(
    "switch.list",
    switchPage.entries.length === 5,
    `names=${switchPage.entries.map((e) => e.name).filter(Boolean).length}/5 named`
  );
  ...
  check(
    "map.events.list",
    Array.isArray(events.entries) || typeof events.total === "number",
    `entries=${(events.entries || []).length} total=${events.total}`
  );
  await send("map.through.set", { value: true });
  assertEqual(
    "map.through.set",
    (await send("map.through.set", { value: false })).through,
    false
  );
  ...
  check(
    "battle.info",
    typeof battle.inBattle === "boolean",
    `inBattle=${battle.inBattle}`
  );
  const kill = await send("battle.killEnemies", {}).catch((e) => e);
  check(
    "battle.killEnemies outside battle",
    kill instanceof Error || (kill && kill.killed === 0),
    kill instanceof Error
      ? kill.message.slice(0, 80)
      : `killed=${kill.killed} remaining=${kill.remaining}`
  );
  ...
  check(
    "lock.list",
    lockList.locks && lockList.locks.gold === 500,
    JSON.stringify(lockList.locks).slice(0, 80)
  );
  ...
  check(
    "save.list",
    saves.entries.length >= 1,
    `dir=${path.basename(saves.dir)} n=${saves.entries.length}`
  );
  ...
  check(
    ...
    saveOk,
    saveOk
      ? `${statSync(saveFile).size} bytes`
      : "file20.rmmzsave never appeared"
  );
  if (existsSync(saveFile)) {
    ...
    await send("console.eval", {
      code: "DataManager.saveGlobalInfo && DataManager.saveGlobalInfo(); 'rebuilt'"
    });
    await sleep(300);
    ...
    check(
      "save.cleanup",
      after.length === 0,
      "file20.rmmzsave removed and did not return"
    );
  }
  ...
  check(
    "save.contents.get",
    contents.bytes > 1000 && (contents.keys || []).includes("party"),
  ...
  const battleMgr = await send("console.eval", {
    code: "typeof window.BattleManager.startBattle"
  });
  ...
  check(
    "assets.iconset",
    (iconset.dataUrl || "").length > 100000 && iconset.width > 0,
