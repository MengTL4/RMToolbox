...
const gameRoot = path.resolve(
  process.argv[2] || path.join(projectRoot, "tmp/storm-196-isolated")
);
...
http
  .createServer(async (req, res) => {
    ...
    try {
      ...
      const session = server.sessions.get(scan.gameKey);
      if (!session) throw Error("No session");
      const out = await session.sendCommand(q.type, q.args || {}, 30000);
      res.end(JSON.stringify({ ok: true, out }));
    } catch (e) {
      res.end(JSON.stringify({ ok: false, error: e.message }));
    }
  })
  .listen(19436, "127.0.0.1", () =>
    console.log("diagnostic endpoint 19436 ready")
  );
