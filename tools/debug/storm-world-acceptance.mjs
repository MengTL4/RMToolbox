...
assert.equal(
  await evaluate("document.hidden"),
  false,
  "keep the game foreground while checking native scene transitions"
);
assert.equal(
  (await send("map.inspect")).busy,
  null,
  "finish the native introduction before testing"
);
...
const combatGrid = await send("map.grid", {
  mapToken: combatMap.mapToken,
  offset: 0,
  count: 2048
});
const freeCell = combatGrid.cells.findIndex(
  (mask, index) =>
    mask === 15 &&
    !combatMap.events.some(
      (event) => event.x === index % 30 && event.y === Math.floor(index / 30)
    )
);
...
await test("map grid and real event reader", async () => {
  ...
  const event = map.events.find((e) => !e.erased);
  assert.ok(event);
...
await test("native selected common-event execution", async () => {
  ...
  try {
    await evaluate(
      `$dataCommonEvents.push({id:${id},name:'RMToolbox isolated execution test',trigger:0,switchId:1,list:[{code:121,indent:0,parameters:[5,5,${previous ? 1 : 0}]},{code:0,indent:0,parameters:[]}]});true`
    );
    const map = await send("map.inspect");
    const plan = await send("events.steps", { mapToken: map.mapToken, id });
    const step = plan.steps.find((s) => s.start === 0);
    assert.ok(step && !step.reason);
    execution = (
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (
        execution &&
        ["completed", "stopped", "failed"].includes(execution.state)
      )
        break;
    }
    ...
    return {
      ...
      fixture: "temporary common event through native interpreter"
    };
  } finally {
    if (
      execution &&
    ...
    await evaluate(
      `$dataCommonEvents.length=${id};$gameSwitches.setValue(5,${JSON.stringify(previous)});true`
    );
  }
});
await test("same-map native transfer", async () => {
  ...
  await sleep(1000);
  const q = await send("player.location");
...
await test("native menu push/pop", async () => {
  ...
  await sleep(600);
  assert.equal((await send("scene.info")).current, "Scene_Menu");
...
await test("no cost and vital locks use native setters", async () => {
  ...
  try {
    ...
    const x = await evaluate(
      "(()=>{const a=$gameParty.members()[0],s=$dataSkills.find(s=>s&&s.mpCost>0);a.setHp(1);a.setTp(1);return {hp:a.hp,mhp:a.mhp,tp:a.tp,mpCost:a.skillMpCost(s),canPay:a.canPaySkillCost(s)}})()"
    );
...
await test("native battle scene", async () => {
  ...
  const backgrounds = await evaluate(
    "({first:$gameMap.battleback1Name(),second:$gameMap.battleback2Name(),hidden:document.hidden})"
  );
  assert.equal(
    backgrounds.hidden,
    false,
    "native scene transitions pause in the background"
  );
  for (const [folder, name] of [
    ["battlebacks1", backgrounds.first],
    ["battlebacks2", backgrounds.second]
  ]) {
    assert.ok(
      name,
      "choose a real combat map with explicit battle backgrounds"
    );
    assert.ok(
      [".png", ".rpgmvp"].some((extension) =>
        fs.existsSync(
          path.join(gameRoot, "www", "img", folder, name + extension)
        )
      ),
      `battle test requires an existing ${folder}/${name} asset; choose a real combat map`
    );
  }
  assert.equal(
    await evaluate(
      "$gamePlayer._encounterCount=0;$gamePlayer.executeEncounter()"
    ),
    true,
    "the current map has a real native encounter"
  );
  await evaluate("SceneManager.push(Scene_Battle);true");
  await sleep(1800);
  assert.equal((await send("scene.info")).current, "Scene_Battle");
  const battle = await send("battle.info");
  assert.equal(battle.inBattle, true);
  assert.ok(battle.enemies.length);
...
if (battleReady) {
  await test("battle gold EXP and drop multipliers", async () => {
    ...
    try {
      ...
      const x = await evaluate(
        "(()=>{const a=$gameParty.battleMembers()[0],g=$gameParty.gold(),v=a.currentExp(),e=$gameTroop.members()[0];$gameParty.gainGold(10,true);a.gainExp(10,true);return {gold:$gameParty.gold()-g,exp:a.currentExp()-v,drop:e.dropItemRate()}})()"
      );
  ...
  await test("invincible native damage", async () => {
    ...
    try {
      ...
      const x = await evaluate(
        "(()=>{const a=$gameParty.battleMembers()[0],e=$gameTroop.members()[0],before=a.hp,action=new Game_Action(e);action.setAttack();action.executeHpDamage(a,10);return {before,after:a.hp}})()"
      );
  ...
  await test("enemy HP command and native one-hit kill", async () => {
    const enemy = (await send("battle.info")).enemies[0];
    await send("battle.enemy.setHp", { index: enemy.index, value: 1 });
    const old = (await send("trainer.options.get")).options;
    try {
  ...
  await test("kill remaining native enemies", async () => {
...
fs.writeFileSync(
  "tmp/storm-world-acceptance.json",
