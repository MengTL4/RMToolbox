...
function evaluate(code) {
  execFileSync(
    process.execPath,
    [new URL("zs2-stairs-probe.mjs", import.meta.url).pathname.slice(1), code],
    { cwd: root, stdio: "pipe" }
  );
  return JSON.parse(
    readFileSync(new URL("runtime/zs2-stairs-probe.json", root), "utf8")
  );
}
...
assert.equal(
  evaluate(snapshotCode).map,
  7,
  "start at village in dedicated test session"
);
for (let i = 1; i <= 3; i++) {
  evaluate(
    `(()=>{var p=TK.$.gamePlayer();p.locate(3,11);p.setDirection(8);p.checkEventTriggerHere([0]);return true})()`
  );
  await until(
    `round ${i}: downstairs`,
    (s) =>
  ...
  await until(
    `round ${i}: upstairs`,
    (s) => s.map === 7 && s.canMove && !s.moving && !s.forcing
  );
  evaluate("(TK.$.gamePlayer().moveStraight(2),true)");
  const walked = await until(
    `round ${i}: input restored`,
    (s) => s.map === 7 && s.y === 12 && s.canMove && !s.moving
  );
  assert.equal(walked.through, false);
  assert.equal(walked.cheat, false);
}
console.log(
  "PASS: 3 complete basement stair round trips; normal collision and player movement restored"
);
