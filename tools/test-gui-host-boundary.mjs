...
const host = createRequire(import.meta.url)(
  path.join(root, "app/gui/host.cjs")
);
let loads = 0;
assert.equal(
  getGuiHost((id) => {
    loads++;
    assert.equal(id, "./host.cjs");
    return host;
  }),
  host
);
...
const contract = ast.statements.find(
  (n) => ts.isInterfaceDeclaration(n) && n.name.text === "GuiHost"
);
for (const member of contract.members)
  assert.equal(
    typeof host[member.name.getText(ast)],
    "function",
    member.name.getText(ast)
  );
console.log(
  "GUI host boundary PASS: clear startup errors, real CJS method contract, no native action during loading"
);
