...
const guiCheck = execSync('powershell -NoProfile -Command "if (Get-CimInstance Win32_Process | Where-Object { $_.Name -match \'RMToolbox\' }) { \'RUNNING\' } else { \'NONE\' }"').toString().trim();
if (guiCheck === "RUNNING") {
...
// itself from zero.
for (const f of ["commands.jsonl", "events.jsonl", "state.json", "catalog-cache.json"]) {
  rmSync(path.join(stateDir, f), { force: true });
}

const scan = scanGame(gameRoot);
elog("scan", `container=${scan.container} exe=${path.basename(scan.paths.exe || "")} key=${scan.gameKey}`);
if (scan.container !== "nb-evalnwbin") throw new Error("unexpected container: " + scan.container);

const summary = await launchNwInjectGame({ scan, projectRoot, port: 47412 });
elog("launch returned", `strategy=${summary.strategy} pid=${summary.pid} injected=${(summary.injected || []).join("/")}`);
...
if (existsSync(cachePath)) {
  ...
  check("cache item non-empty", Array.isArray(cache.tables.item) && cache.tables.item.length > 1,
    `len=${cache.tables.item && cache.tables.item.length}`);
  check("cache mapInfo non-empty", Array.isArray(cache.tables.mapInfo) && cache.tables.mapInfo.length > 1,
...
check("catalog.query item total>0", !!(cat.ok && cat.payload && cat.payload.total > 0),
...
if (cat.ok && cat.payload && cat.payload.total > 0) {
  ...
  elog("sample items", cat.payload.entries.slice(0, 5).map((e) => `${e.id}:${e.name}`).join(" | "));
}
for (const kind of ["weapon", "armor", "actor", "commonEvent"]) {
  const r = await cmd("catalog.query", { kind, limit: 20000 });
  check(`catalog.query ${kind} total>0`, !!(r.ok && r.payload && r.payload.total > 0),
...
check("switch.list resolves $dataSystem", !!(sw.ok && sw.payload && sw.payload.namesUnavailable === false
  && sw.payload.entries.length > 0),
  sw.ok ? `namesUnavailable=${sw.payload.namesUnavailable} entries=${sw.payload.entries.length}` : sw.error);
const vr = await cmd("variable.list", { limit: 2000 });
check("variable.list with names", !!(vr.ok && vr.payload && vr.payload.namesUnavailable === false
...
check("map.list total>0", !!(mp.ok && mp.payload && mp.payload.total > 0),
  `total=${mp.payload && mp.payload.total}`);
const party = await cmd("party.info", {});
elog("party.info at title (capture fills after the user loads a save)",
...
while (Date.now() < watchDeadline) {
  ...
  const out = execSync('powershell -NoProfile -ExecutionPolicy Bypass -File "runtime/_scratch/_is-game-alive.ps1"', { cwd: projectRoot }).toString().trim();
  if (out !== "ALIVE") { alive = false; break; }
}
