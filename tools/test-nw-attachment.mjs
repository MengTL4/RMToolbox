...
function fixture(family, options = {}) {
  ...
  const clock = {
    ...
    sleep: async (ms) => {
      ...
      if (
        context.compatReadyAt &&
        context.now >=
          context.compatReadyAt + (options.compatibilityLateMs || 0) &&
        !options.compatibilityNeverReady &&
        (!options.lateWindow || context.windowShown)
      ) {
      ...
      if (options.capture !== false) {
        for (const command of context.commands()) {
          if (
            command.type !== "system.rebootCapture" ||
            context.captureReplies.has(command.commandId)
          )
            continue;
          assert.ok(
            context.injections.some(
              (entry) => entry.env.RMCH_THROW_IF_BRIDGED === "1"
            ),
          ...
          writeFileSync(
            ...
            JSON.stringify({
              ...
              tables: Object.fromEntries(
                ["actor", "skill", "item", "weapon", "armor", "state"].map(
                  (key) => [key, [null, { id: 1 }]]
                )
              )
            })
  ...
  const platform = {
    listProcessModules: async (pid) => {
      ...
      if (race && context.moduleQueries <= (race.failures || 1)) {
        if (race.replaceRenderer && context.moduleQueries === 1) {
          context.processes = [
            processRow(101, ""),
            processRow(202, "--type=renderer")
          ];
        }
        throw new AttachError(
          "process query failed: Cannot find a process with the process identifier"
        );
      }
    ...
    cmdStartSpawn: (...args) => {
      ...
      assert.equal(
        args.length,
        3,
        "plain launch must not gain extraEnv or flags"
      );
    ...
    injectAndDeliver: async (request) => {
      ...
      if (request.bootstrap.includes("__rmchGroverCompatibility")) {
        assert.deepEqual(
          context.commands(),
          [],
          "cold compatibility injection must not replay old commands"
        );
        assert.equal(
          existsSync(eventPath),
          false,
          "old hello/results are cleared before delayed bridge starts"
        );
        assert.equal(
          existsSync(statePath),
          false,
          "old state cannot satisfy delayed readiness"
        );
        assert.ok(
          Buffer.byteLength(request.bootstrap) < 16000,
          "early payload must reference the bridge file, never embed the full bridge source"
        );
        ...
        if (options.deliveryLateMs) {
          ...
          if (request.timeoutMs < options.deliveryLateMs)
        ...
        if (options.compatibilityStalePid && request.pid !== 202) {
          ...
          return {
            ok: false,
            detail: "injector-exit-2: OpenProcess failed: 87"
          };
        }
        if (
          options.compatibilityTimeoutRace &&
          context.injections.filter((entry) => entry.compatibility).length <=
            (options.compatibilityTimeoutRaceFailures || 1)
        ) {
          context.processes = [
            processRow(101, ""),
            processRow(202, "--type=renderer")
        ...
        if (options.compatibilityFailure)
          return { ok: false, detail: "compatibility probe failed" };
              ...
              : name === "timers"
                ? {
                    setTimeout: (_callback, delay) => {
                      context.compatReadyAt = context.now + delay;
                    }
                  }
                : path,
          nw: { Window: {} }
...
try {
  ...
  for (const family of ["nb-evalnwbin", "enigma-nb", "grover"]) {
    for (const operation of ["attach", "existing", "launched"]) {
      ...
      const result = await (operation === "attach"
        ? game.attach()
        : game.launch());
      const file =
        operation === "existing" ||
      ...
      assert.equal(
        first.env.RMCH_BOOT_TAP,
        undefined,
        "normal attachment never installs the boot tap"
      );
      assert.equal(
        first.timeoutMs,
        file && operation === "launched" ? 10000 : 30000
      );
      assert.equal(
        result.strategy,
        operation === "launched"
      ...
      assert.deepEqual(
        game.spawns,
        operation === "launched" ? ["primary"] : []
      );
      assert.equal(
        game.commands().length,
        file && operation === "launched" ? 1 : 0,
        "only a file game actually spawned in this call may reload for capture"
      );
      assert.equal(
        game.injections.filter((entry) => entry.env.RMCH_BOOT_TAP === "1")
          .length > 0,
  ...
  for (const [family, launch, attempts, timeout, existing] of [
    ["nwjs", false, 1, 30000],
    ["enigma-nb", false, 5, 30000],
    ["enigma-nb", true, 8, 30000],
    ["nb-evalnwbin", true, 8, 10000],
    ...
    await assert.rejects(
      launch ? game.launch() : game.attach(),
      /injection failed/
    );
  ...
  const fallback = fixture("enigma-nb", {
    running: false,
    primaryAppears: false
  });
  ...
  assert.match(
    readFileSync(path.join(queryRace.stateDir, "bridge.log"), "utf8"),
    /process poll error/
  );
  checks += 1;
  ...
  await assert.rejects(
    noRenderer.launch(),
    /no renderer process appeared within 90s/
  );
  ...
  const exitedBeforeRetry = fixture("nb-evalnwbin", {
    ...
    onSleep: (ms, game) => {
      if (ms === 4000) game.processes = [];
    }
  });
  await assert.rejects(
    exitedBeforeRetry.launch(),
  ...
  for (const duringSettle of [false, true]) {
    ...
    await assert.rejects(
      game.launch(),
      duringSettle ? /before attach/ : /exited during boot/
    );
  ...
  assert.equal(
    partialCatalog.commands().length,
    1,
    "partial cache must retry early capture for missing actor and skill tables"
  );
  checks += 1;
  ...
  assert.equal(
    noCatalog.commands().length,
    2,
    "capture remains bounded and best-effort"
  );
  ...
  for (const modules of [
    [],
    [knownModule],
    ["D:\\MacType\\MacType.dll", "D:\\MacType\\MacType.Core.dll"]
  ]) {
    const game = fixture("grover", { running: false, modules });
    if (!modules.some((value) => value.includes("MacType")))
      game.scan.protection.flags.push("grover-module-monitor");
    if (modules.length) {
      mkdirSync(game.stateDir, { recursive: true });
      writeFileSync(
        game.commandPath,
        JSON.stringify({ commandId: "f1", type: "game.newGame", args: {} }) +
          "\n"
      );
    ...
    if (modules.length) {
      assert.equal(
        game.injections.length,
        1,
        "one native delivery; no reload loses the compatibility layer"
      );
      assert.equal(game.injections[0].compatibility, true);
      assert.equal(
        game.windowShown,
        true,
        "restore the known game's native window for its first animation frame"
      );
      assert.ok(
        game.now - game.injections[0].at < 5000,
        "a ready scene must not accumulate uncapped frames in a fixed settle delay"
      );
      const status = JSON.parse(
        readFileSync(path.join(game.stateDir, "grover-compat.json"), "utf8")
      );
      assert.equal(status.status, "applied");
    } else {
      assert.ok(
        game.injections.every((i) => !i.compatibility),
        "without actual known modules the old route is unchanged"
      );
    }
    checks++;
  }
  const lateDelivery = fixture("grover", {
    running: false,
    modules: [knownModule],
    deliveryLateMs: 19000
  });
  await lateDelivery.launch();
  assert.equal(
    lateDelivery.injections.length,
    1,
    "wait for the native hook result without injecting twice"
  );
  checks++;
  const outerLauncher = fixture("grover", {
    running: false,
    modules: [knownModule],
    outerLauncher: true
  });
  ...
  assert.equal(
    outerLauncher.windowShown,
    true,
    "restore NW's game window even when an outer launcher appears first"
  );
  checks++;
  const lateWindow = fixture("grover", {
    running: false,
    modules: [knownModule],
    lateWindow: true
  });
  await lateWindow.launch();
  assert.equal(
    lateWindow.windowShown,
    true,
    "retry window restoration after renderer discovery precedes window creation"
  );
  checks++;
  const compatFailure = fixture("grover", {
    running: false,
  ...
  assert.ok(
    compatFailure.injections.every((i) => i.compatibility),
    "never install full bridge after failed preparation"
  );
  checks++;
  ...
  writeFileSync(
    liveCompat.commandPath,
    JSON.stringify({ commandId: "previous-session", type: "ping", args: {} }) +
      "\n"
  );
  await liveCompat.launch();
  assert.equal(
    liveCompat.commands()[0].commandId,
    "previous-session",
    "existing live file session is never cleared"
  );
  assert.equal(
    liveCompat.injections.length,
    0,
    "existing hello is reused without another native hook"
  );
  checks++;
  const staleCompat = fixture("grover", {
    running: false,
    modules: [knownModule],
    compatibilityStalePid: true
  });
  staleCompat.scan.protection.flags.push("grover-module-monitor");
  await staleCompat.launch();
  assert.equal(
    staleCompat.injections.at(-1).pid,
    202,
    "only a failed OpenProcess permits refreshing the replacement renderer"
  );
  checks++;
  ...
  assert.equal(
    moduleQueryRace.moduleQueries >= 2,
    true,
    "retry module inspection after a renderer replacement"
  );
  assert.equal(
    moduleQueryRace.injections.find((entry) => entry.compatibility).pid,
    202
  );
  assert.ok(
    !moduleQueryRace.sleeps.includes(60000),
    "a transient module query race must not fall into the 60s fallback"
  );
  assert.match(
    readFileSync(path.join(moduleQueryRace.stateDir, "bridge.log"), "utf8"),
    /module query/
  );
  checks++;
  ...
  assert.deepEqual(
    compatibilityTimeoutRace.injections
  ...
  for (const never of [false, true]) {
    const game = fixture("grover", {
      running: false,
      modules: [knownModule],
      compatibilityLateMs: 20000,
      compatibilityNeverReady: never
    });
    game.scan.protection.flags.push("grover-module-monitor");
    if (never)
      await assert.rejects(
        game.launch(),
        /delayed game bridge did not become ready/
      );
    ...
    if (never)
      assert.equal(
        game.injections.length,
        1,
        "timeout does not fall through to another DLL injection"
      );
    checks++;
  }
