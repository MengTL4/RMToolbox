...
function check(name, actual, expected) {
  ...
  const b = Buffer.isBuffer(expected)
...
const golden = (name) =>
  Buffer.from(fixtures.cases.find((c) => c.name === name).hex, "hex");
...
for (const entry of fixtures.cases.filter((c) =>
  c.name.startsWith("fixnum_")
)) {
  ...
  check(
    `int encode ${entry.name}`,
    encodeInt(expectedValue).toString("hex"),
    expected
  );
}
...
// produced and check that decode -> encode reproduces its bytes.
{
  ...
  const wrapped = Buffer.concat([
    Buffer.from([0x04, 0x08, 0x5b, 0x06]),
    triple
  ]);
  ...
  const rebuilt = encodeScriptEntry(entry.id, entry.name, entry.zlib, {
    ruby19: true
  });
  check(
    "triple re-encodes byte-identically",
    rebuilt.toString("hex"),
    triple.toString("hex")
  );
}
...
// --- 3. insert must match Ruby byte for byte -------------------------------
{
  ...
  const spliced = insertScriptEntry(two, 2, last.id, last.name, last.zlib, {
    ruby19: true
  });
  ...
  ok(
    "spliced entry count matches Ruby",
    splicedParsed.count === threeParsed.count
  );
  for (let i = 0; i < threeParsed.count; i += 1) {
    ok(
      `entry ${i} id matches`,
      splicedParsed.entries[i].id === threeParsed.entries[i].id
    );
    ok(
      `entry ${i} name matches`,
      splicedParsed.entries[i].name === threeParsed.entries[i].name
    );
    check(
  ...
  check(
    "Ruby's first two entries survive byte-identically",
    ourPrefix.toString("hex"),
    rubyPrefix.toString("hex")
  );
...
// --- 4. splice preserves untouched entries byte for byte --------------------
{
  ...
  const spliced = insertScriptEntry(two, 1, 42, "Bridge", payload, {
    ruby19: true
  });
  ...
  const rawBefore = two.subarray(
    before.entries[0].entryStart,
    before.entries[0].entryEnd
  );
  const rawAfter = spliced.subarray(
    after.entries[0].entryStart,
    after.entries[0].entryEnd
  );
  check(
    "entry 0 bytes unchanged",
    rawAfter.toString("hex"),
    rawBefore.toString("hex")
  );
  // Entry 1 (now index 2) must also be untouched.
  const rawMoved = spliced.subarray(
    after.entries[2].entryStart,
    after.entries[2].entryEnd
  );
  const rawOrig1 = two.subarray(
    before.entries[1].entryStart,
    before.entries[1].entryEnd
  );
  check(
    "entry 1 bytes unchanged after move",
    rawMoved.toString("hex"),
    rawOrig1.toString("hex")
  );
}
...
if (sampleDir && existsSync(sampleDir)) {
  const samples = [
    ...
    {
      file: "knight-blade/KN_E/Data/Scripts.rxdata",
      ruby19: false,
      label: "XP"
    },
    {
      file: "leg-x/Legionwood Tale of the Two Swords/Data/Scripts.rvdata",
      ruby19: false,
      label: "VX"
    },
    {
      file: "hs-scripts.rvdata2",
      ruby19: true,
      label: "VX Ace (encrypted archive)"
    }
  ];
  for (const sample of samples) {
    ...
    ok(
      ...
      zlib.inflateSync(reparsed.entries[parsed.count].zlib).toString("utf8") ===
        "# rmch bridge\n"
    );
    // Original entries must survive untouched.
    let drift = 0;
    for (let i = 0; i < parsed.count; i += 1) {
      const a = buf.subarray(
        parsed.entries[i].entryStart,
        parsed.entries[i].entryEnd
      );
      const b = spliced.subarray(
        reparsed.entries[i].entryStart,
        reparsed.entries[i].entryEnd
      );
      if (a.toString("hex") !== b.toString("hex")) drift += 1;
    }
    ok(
      `${sample.label}: no byte drift in ${parsed.count} entries`,
      drift === 0,
      `drift=${drift}`
    );
  }
...
if (failures.length) {
  console.error(
    `rgss-marshal test: FAIL (${failures.length} of ${passed + failures.length})`
  );
