...
await ev('($scene.instance_variable_get(:@eventscene).class.to_s rescue "none")');
// dispose it directly and watch what happens
await ev('es = $scene.instance_variable_get(:@eventscene); (es.dispose rescue "ERR #{$!.class}"); es.disposed?.inspect rescue "?"');
await sleep(2000);
await ev('$scene.class.to_s');
