...
try {
  ...
  const state = await poll(
    () => (session.state && session.state.saveDir ? session.state : null),
    5000
  );
  ...
  if (essentials) {
    ...
    const entry =
      list0.entries.find((e) => SLOT_RE_ESS.test(e.name)) || list0.entries[0];
  ...
  check(
    "save.save",
    saved.saved === true && saved.id === SLOT,
    JSON.stringify(saved)
  );

  const list1 = await send("save.list");
  const entry = list1.entries.find(
    (e) =>
      Number((SLOT_RE.exec(e.name) ?? SLOT_RE_ESS.exec(e.name))?.[1]) === SLOT
  );
  check(
    "save.list shows slot",
    !!entry,
    entry ? `${entry.name} ${entry.size}B` : "not listed"
  );
  if (entry) {
  ...
  check(
    "save.load",
    loaded.loaded === true && loaded.id === SLOT,
  ...
  check(
    "load restores switch",
    !!sw && sw.value === false,
    sw ? `switch1=${sw.value}` : "no switch list"
  );

  // --- error path ------------------------------------------------------------------
  const missing = await send("save.load", { id: 98 }).then(
    () => "",
    (e) => e.message
  );
  check(
    "load missing slot refuses",
    /not found|out of range/.test(missing),
    missing
  );
...
console.log(
  failures ? `rgss-saves: FAIL (${failures} checks)` : "rgss-saves: PASS"
);
