...
await session.send("save.load", { id: 1 }, 30000).then((p) => console.log("loaded", JSON.stringify(p)));
await sleep(3000);
await ev('caller.first(22).join("\n")');
await ev('$scene.class.to_s + " / " + ($scene.instance_variable_get(:@eventscene).disposed?.inspect rescue "-")');
