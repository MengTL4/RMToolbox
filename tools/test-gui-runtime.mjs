...
import { readRuntimeLock, verifyInstalledRuntime } from '../core/setup-gui-runtime.mjs';
...
assert.ok(verifyInstalledRuntime(runtime, lock), 'Runtime must match the pinned manifest');
...
const stamp = JSON.parse(fs.readFileSync(path.join(runtime, '.nw-runtime.json'), 'utf8'));
...
const manifest = JSON.parse(fs.readFileSync(path.join(gui, 'package.json'), 'utf8'));
fs.writeFileSync(path.join(temp, 'package.json'), JSON.stringify({...manifest, name: 'rmch-runtime-smoke', main: 'index.html', window: {...manifest.window, show: false}}));
fs.writeFileSync(path.join(temp, 'index.html'), fs.readFileSync(path.join(gui, 'index.html'), 'utf8').replace('<head>', '<head>' + intro).replace('<script src="ui/main.js"></script>', smoke));
const child = spawn(path.join(temp, 'RMToolbox.exe'), ['--headless', '--disable-gpu', `--user-data-dir=${path.join(temp, 'profile')}`], {windowsHide: true, stdio: 'ignore'});
let spawnError;
child.on('error', error => { spawnError = error; });
try {
  const deadline = Date.now() + 45000;
  while (!fs.existsSync(resultFile) && Date.now() < deadline && child.exitCode === null && !spawnError) await new Promise(resolve => setTimeout(resolve, 100));
  if (spawnError) throw spawnError;
  assert.ok(fs.existsSync(resultFile), 'NW smoke did not write a result (exit ' + child.exitCode + ')');
  const result = JSON.parse(fs.readFileSync(resultFile, 'utf8'));
  assert.equal(result.ok, true, JSON.stringify(result));
  console.log('NW smoke PASS ' + result.versions.nw + ': ' + result.checks.join(', '));
} finally {
  if (child.exitCode === null) child.kill();
  await new Promise(resolve => { if (child.exitCode !== null) resolve(); else { child.once('exit', resolve); setTimeout(resolve, 2000).unref(); } });
  // mkdtemp creates this exact owned directory; never follow a linked replacement.
  if (!fs.lstatSync(temp).isSymbolicLink() && path.dirname(temp) === path.resolve(os.tmpdir())) {
    ...
    await fs.promises.rm(temp, {recursive: true, force: true, maxRetries: 10, retryDelay: 500});
  }
}
