...
const evalRb = async (code) =>
  (await session.send("console.eval", { code }, 30000)).result;
...
await evalRb(
  "class << Input; alias_method :rmch_dbg_orig_trig, :trigger? unless method_defined?(:rmch_dbg_orig_trig); " +
    "define_method(:trigger?) do |*a|; true; end; end; 'page-on'"
).then((r) => console.log("page-on:", r));
await sleep(3500);
await evalRb(
  "class << Input; if method_defined?(:rmch_dbg_orig_trig); alias_method :trigger?, :rmch_dbg_orig_trig; " +
    "remove_method :rmch_dbg_orig_trig; end; end; 'page-off'"
).then((r) => console.log("page-off:", r));
await sleep(500);
...
const menuPromise = session
  .send("console.eval", { code: "pbDebugMenu; 'menu-closed'" }, 120000)
  .then(
...
console.log(
  "menu scene check:",
  await evalRb("$scene ? $scene.class.to_s : 'nil'").catch(() => "?")
);
...
await evalRb(
  "class << Input; alias_method :rmch_dbg_trigger, :trigger? unless method_defined?(:rmch_dbg_trigger); " +
    "define_method(:trigger?) do |s|; true if s == Input::B; end; end; 'cancel-on'"
).catch(() => null);
const verdict = await Promise.race([
  menuPromise,
  sleep(15000).then(() => "menu still open after 15s")
]);
console.log(verdict);
