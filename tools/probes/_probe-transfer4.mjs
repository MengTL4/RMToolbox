...
await ev(
  'class Scene_Map; alias $__probe_base_update update unless method_defined?($__probe_base_update); end; "no" rescue "alias-skip"'
);
await ev(
  '$__upd_calls = 0; Scene_Map.prepend(Module.new do; def update; $__upd_calls += 1; super; end; end); "prepended"'
);
await sleep(1500);
await ev("$__upd_calls.to_s");
// Now drive the flags and watch whether transfer_player gets called
await ev(
  '$__tp_calls = 0; Scene_Map.prepend(Module.new do; def transfer_player(*a); $__tp_calls += 1; super; end; end); "prepended2"'
);
await session
...
await sleep(2000);
await ev(
  "[$__upd_calls, $__tp_calls, $game_temp.player_transferring, $game_map.map_id].inspect"
);
// Manual direct call as fallback
await ev(
  '$scene.transfer_player(false) rescue "ERR #{$!.class}: #{$!.message}"; $game_map.map_id.to_s'
);
await sleep(1500);
