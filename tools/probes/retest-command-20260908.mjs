...
const token = getToken(
  session.projectRoot ||
    process.env.RMCH_COMPARE_PROJECT ||
    path.resolve(import.meta.dirname, "..", "..")
);
const socket = new WebSocket(
  `ws://127.0.0.1:${session.port}/client?token=${encodeURIComponent(token)}`
);
...
socket.onmessage = (e) => {
  ...
  if (r.t === "result") {
    console.log(JSON.stringify(r.ok ? r.payload : { error: r.error }));
    clearTimeout(timer);
    socket.close();
