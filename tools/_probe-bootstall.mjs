...
const BOOTSTRAPS = {
  ...
  tap: "(function(){ if (window.__rmchBootTap) return 'already'; window.__rmchBootTap = true;"
...
if (!bootstrap) { console.error("unknown variant", variant); process.exit(2); }
...
while (Date.now() - t0 < 60000 && !injected) {
  ...
  const result = await injectAndDeliver({
    projectRoot, arch: readPeArch(EXE), pid: pids[0], dllName: "rmch-mvhook.dll",
    bootstrap, mode: "crt", timeoutMs: 25000
  });
  console.log(`[+${((Date.now() - t0) / 1000).toFixed(1)}s] inject(${variant}):`, JSON.stringify(result));
  injected = result.ok;
  if (!injected) await new Promise((r) => setTimeout(r, 1000));
}
if (!injected) { console.log("INJECTION NEVER LANDED"); process.exit(1); }
