...
function makeV1Archive(filePath, files, versionByte = 0x01) {
  ...
  let key = 0xdeadcafe;
  const parts = [
    Buffer.from([0x52, 0x47, 0x53, 0x53, 0x41, 0x44, 0x00, versionByte])
  ];
...
const v1Scripts = Buffer.concat([
  Buffer.from([0x04, 0x08]),
  Buffer.from("fake-scripts-payload-0123456789", "latin1")
]);
...
check(
  "v1 extract round-trips the first entry",
  extractEntry(v1Archive, SCRIPTS_V1),
  v1Scripts
);
check(
  "v1 extract round-trips the second entry (chain position)",
  extractEntry(v1Archive, ITEMS_V1),
  v1Items
);

// Patch with a DIFFERENT length: the tail copy must shift and the trailing
// entry must still decrypt.
const v1Patched = Buffer.concat([
  Buffer.from([0x04, 0x08]),
  Buffer.from("patched-with-rmch-bridge-aaaaaaaaaa", "latin1")
]);
const v1Out = path.join(v1Dir, "out.rgssad");
const v1Result = patchEntry({
  src: v1Archive,
  dst: v1Out,
  entry: SCRIPTS_V1,
  data: v1Patched
...
ok(
  "v1 patch recorded the new size",
  v1Result.size === v1Patched.length,
  `size=${v1Result.size}`
);
check(
  "v1 patched payload reads back",
  extractEntry(v1Out, SCRIPTS_V1),
  v1Patched
);
check(
  "v1 untouched entry survives patching",
  extractEntry(v1Out, ITEMS_V1),
  v1Items
);
...
let v2Threw = "";
try {
  extractEntry(v2Archive, SCRIPTS_V1);
} catch (e) {
  v2Threw = e.message;
}
...
try {
  patchEntry({
    src: v2Archive,
    dst: path.join(v1Dir, "out2.rgss2a"),
    entry: SCRIPTS_V1,
    data: Buffer.alloc(1)
  });
} catch (e) {
...
if (!hasSamples) {
  console.log(
    "rgss-archive test: SKIP v3 section (set RMCH_RGSS_SAMPLES with hs/Game.rgss3a)"
  );
}
...
if (hasSamples) {
  ...
  ok(
    "archive only grew by the payload",
    statSync(patched).size - statSync(archive).size === marker.length
  );
...
if (failures.length) {
  console.error(
    `rgss-archive test: FAIL (${failures.length} of ${passed + failures.length})`
  );
