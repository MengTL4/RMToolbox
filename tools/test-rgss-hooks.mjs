...
const AC_OFF =
  "module ::Input; class << self; if method_defined?(:rmch_ac_trig) && $rmch_ac_method; " +
...
try {
  ...
  check(
    "options.set nested",
    set.options.invincible === true &&
      set.options.expRate === 2.5 &&
      set.options.gameSpeedMulti === 3,
    JSON.stringify({
      inv: set.options.invincible,
      exp: set.options.expRate,
      spd: set.options.gameSpeedMulti
    })
  );
  ...
  check(
    "hooks installed",
    missing.length === 0,
    `${hooks.length} hooks${missing.length ? ", missing " + missing.join(",") : ""}`
  );
  ...
  check(
    "lock.replace nested",
    list.locks.gold === 777 &&
      list.locks.item["2"] === 88 &&
      list.locks.switch["5"] === true &&
      list.locks.variable["7"] === -3,
  ...
  check(
    "lock.clear all",
    list.locks.gold === null && Object.keys(list.locks.item).length === 0,
    ""
  );
  ...
  check(
    "battle.info (title)",
    battle.inBattle === false && battle.enemies.length === 0,
    ""
  );
  const escapeError = await send("battle.escape", {}).then(
    () => "",
    (e) => e.message
  );
  check(
    "battle.escape refuses",
    /not in battle/.test(escapeError),
    escapeError
  );
  ...
  check(
    "new game started",
    !!party,
    party ? `${party.members.length} members, gold ${party.gold}` : "no party"
  );
  ...
  const actorRef =
    gen === "RGSS1" ? "$game_party.actors[0]" : "$game_party.members[0]";
  ...
  await send("trainer.options.set", {
    options: { lockHp: true, lockHpVal: lockVal }
  });
  ...
  check(
    "lockHp clamps write",
    hpNow === lockVal || (mhp < lockVal && hpNow === mhp),
    `hp=${hpNow}, want ${lockVal}`
  );
  await send("trainer.options.set", {
    options: { lockHp: false, lockHpVal: 0 }
  });
  ...
  // --- world options -------------------------------------------------------------
  await send("trainer.options.set", {
    options: { throughWalls: true, noEncounter: true }
  });
  await sleep(300);
  ...
  check(
    "throughWalls forces @through",
    String(through) === "true",
    `@through=${through}`
  );
  check(
    "noEncounter forces flag",
    String(noEnc) === "true",
    `encounter_disabled=${noEnc}`
  );
  await send("trainer.options.set", {
    options: { throughWalls: false, noEncounter: false }
  });
  await sleep(300);
  const throughOff = await evalRb(
    "$game_player.instance_variable_get(:@through)"
  );
  const encOff = await evalRb("$game_system.encounter_disabled");
  check(
    "options off restore",
    String(throughOff) !== "true" && String(encOff) !== "true",
  ...
  if (gen === "RGSS3") {
    ...
    await evalRb(
      `$data_system.title_bgm.play if RPG::BGM.last.nil?; ` +
    ...
    await evalRb(
      `$game_troop.setup(${troopId}); $game_troop.can_escape = true; ` +
    ...
    await evalRb(
      `$game_temp.battle_troop_id = ${troopId}; $game_temp.battle_can_escape = true; ` +
  ...
  check(
    "battle entered",
    !!battle && battle.enemies.length > 0,
    battle
      ? `troop=${troopId}, enemies=${battle.enemies.map((e) => `${e.name} ${e.hp}/${e.mhp}`).join(" | ")}`
      : "no battle"
  );
  if (!battle || !battle.enemies.length)
    throw new Error("cannot continue without a battle");
  ...
  const enemy0Ref =
    (gen === "RGSS1" ? "$game_troop.enemies[0]" : "$game_troop.members[0]") +
    ".hp";
  try {
    ...
    if (gen === "RGSS3") {
      await evalRb(
        `$game_troop.members[0].item_apply($game_party.members[0], $data_items[1]); ${AC_ON}; 'applied'`
      );
    } else if (gen === "RGSS2") {
      await evalRb(
        `$game_troop.members[0].attack_effect($game_party.members[0]); ${AC_ON}; 'applied'`
      );
    } else {
      await evalRb(
        `$game_troop.enemies[0].attack_effect($game_party.actors[0]); ${AC_ON}; 'applied'`
      );
    }
    await sleep(300);
    ...
    check(
      "oneHitKill fells enemy",
      false,
      `${error.message}; bridge alive=${alive}`
    );
  ...
  const expExpr =
    gen === "RGSS1" ? "$game_troop.enemies[0].exp" : "$game_troop.exp_total";
  ...
  check(
    "expRate scales",
    expBase > 0 ? expScaled === expBase * 2 : expScaled === 0,
  ...
  check(
    "battle.killEnemies",
    killed.remaining === 0,
    `killed=${killed.killed} remaining=${killed.remaining}`
  );
  ...
  if (!left || left.inBattle !== false) {
    if (gen === "RGSS3")
      await evalRb("BattleManager.process_abort; 'forced'").catch(() => {});
    else
      await evalRb(
        "$scene.battle_end(0) if $scene.respond_to?(:battle_end); 'forced'"
      ).catch(() => {});
    ...
    check(
      "battle ends",
      !!after && after.inBattle === false,
      after ? "" : "still in battle"
    );
  } else {
  ...
  check(
    "state push carries options",
    !!(
      state &&
      state.options &&
      typeof state.options.invincible !== "undefined"
    ),
    state && state.options
      ? `invincible=${state.options.invincible}`
      : "no state"
  );
} catch (error) {
  failures += 1;
  ...
  await send("trainer.options.set", {
    options: { invincible: false, oneHitKill: false, lockHp: false }
  }).catch(() => {});
  handle.stop();
}

console.log(
  failures ? `rgss-hooks: FAIL (${failures} checks)` : "rgss-hooks: PASS"
);
