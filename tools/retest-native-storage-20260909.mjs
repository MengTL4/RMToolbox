...
socket.onmessage=e=>{const r=JSON.parse(String(e.data));const p=pending.get(r.id);if(r.t==='result'&&p){pending.delete(r.id);clearTimeout(p.timer);r.ok?p.resolve(r.payload):p.reject(Error(r.error));}};
const send=(type,args={})=>new Promise((resolve,reject)=>{const id='storage-'+(++serial);const timer=setTimeout(()=>{pending.delete(id);reject(Error('timeout '+type));},35000);pending.set(id,{resolve,reject,timer});socket.send(JSON.stringify({t:'send',id,gameKey:session.gameKey,type,args}));});
try {
 const fixtureState={projectRoot:session.projectRoot,modules:{saveFiles:{createSaveFiles}},sessions:{
  list:()=>[{gameKey:session.gameKey,alive:true,state:{saveStorage:storage}}],send:(_key,type,args)=>send(type,args)}};
 ...
 assert.equal(host.listBackups(session.gameKey).find(x=>x.name===name).storage,storage);
 await host.restoreBackup(session.gameKey,name);
 const after=await send(exportCommand);
 assert.deepEqual(after.entries,before.entries);
 console.log('PASS native GUI host backup/list/restore exact contents',JSON.stringify({files:backup.files,destDir:backup.destDir}));
