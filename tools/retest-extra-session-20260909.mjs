...
const save = () =>
  fs.writeFileSync(
    output,
    JSON.stringify({ gameKey: session.gameKey, checks }, null, 2)
  );
const test = async (name, fn) => {
  try {
    ...
    const status = /暂不支持数量锁|暂不支持额外游戏倍速|独立.*不支持.*锁/.test(
      e.message
    )
      ? "limitation"
      : "fail";
...
const suite = process.argv.includes("--effects")
  ? (await import("./retest-effects-20260908.mjs")).effects
  : extra;
try {
  await suite({
    send,
    ev: async (code) => (await send("console.eval", { code })).result,
    test,
    blocked,
