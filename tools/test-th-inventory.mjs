...
const tables = {
  item: [null, { id: 1, kind: "item", itypeId: 1, name: "药", meta: {} }],
  weapon: [null, { id: 1, kind: "weapon", wtypeId: 1, name: "枪", meta: {} }],
  armor: [null, { id: 1, kind: "armor", atypeId: 1, name: "衣", meta: {} }]
};
const actor = { _newItemList: [], actorId: () => 1 },
  pilot = { _newItemList: ["I1"], actorId: () => 2 };
const party = {
  _tkCkItem: ["I1"],
  _cxJsId: { 1: 2 },
  members: () => [actor, pilot, actor],
  thTyZhNumGain() {},
  thTyZhNumGet() {},
  newGetItem(data) {
    const owner =
      actor._newItemList.length < 3 ? actor._newItemList : party._tkCkItem;
    if (owner === party._tkCkItem && owner.length >= 4) return;
    let value = data;
    if (data.kind !== "item") {
      value = { ...data, id: tables[data.kind].length, baseItemId: data.id };
      tables[data.kind].push(value);
    }
    owner.push({ item: "I", weapon: "W", armor: "A" }[data.kind] + value.id);
  },
  gainItem(data, delta, includeEquip, owner) {
    const token = { item: "I", weapon: "W", armor: "A" }[data.kind] + data.id;
    const index = owner._newItemList.indexOf(token);
    if (index >= 0) owner._newItemList.splice(index, 1);
    if (data.baseItemId) tables[data.kind][data.id] = null;
  }
};
const window = {
  DataManager: {
    isItem: (x) => tables.item.includes(x),
    isWeapon: (x) => tables.weapon.includes(x),
    isArmor: (x) => tables.armor.includes(x)
  },
  $gameActors: { actor: (id) => (id === 2 ? pilot : actor) }
};
const context = { window, runtimeDataTable: (kind) => tables[kind] };
vm.runInNewContext(
  fs.readFileSync("runtime/bridge/src/parts/31-th-inventory.js", "utf8"),
  context
);
const adapter = context.resolveCustomInventory(party),
  item = tables.item[1];
...
assert.throws(
  () => adapter.change({ ...item, meta: { tkPaoDang: true } }, 1),
  /独立弹仓/
);
...
console.log(
  "PASS TH inventory owners, warehouse, native mutation, capacity, partial progress, instance protection"
);
...
const independent = {
  ...
  gainIndependentItem() {},
  numItems(data) {
    return this._weapons[data.id] || 0;
  },
  gainItem(data, amount) {
    if (amount > 0)
      for (let n = 0; n < amount; n++) {
        const row = { ...data, id: "I" + ++this.serial, baseItemId: data.id };
        this.owned.push(row);
        this._weapons[row.id] = 1;
      }
    else {
      this.owned = this.owned.filter((row) => row !== data);
      delete this._weapons[data.id];
    }
  },
  serial: 100
};
...
assert.equal(
  independent._weapons[weapon.id],
  undefined,
  "never fabricate numeric base stock"
);
const nativeRows = nativeAdapter.entries();
assert.equal(nativeRows[0].id, "I101");
assert.equal(nativeRows[0].baseItemId, 1);
assert.equal(
  nativeAdapter.change(weapon, -1),
  1,
  "base aggregate removal must delete one native instance"
);
assert.doesNotThrow(() => nativeAdapter.validateLock(weapon, 1));
assert.throws(
  () => nativeAdapter.validateLock(independent.owned[0], 2),
  /0 或 1/
);
...
console.log(
  "PASS native independent string IDs, aggregate base count and precise removal"
);
...
const encoded = {
  ...
  items() {
    return Object.keys(this._items)
      .filter((id) => this._items[id])
      .map((id) => encodedItems[Number(id)]);
  },
  weapons() {
    return [];
  },
  ...
  numItems(data) {
    return this.thTyZhNumGet(this._items[data.id]);
  },
  newNumItems() {
    return 0;
  },
  ...
  thTyZhNumGet(value) {
    return value == null ? 0 : Number(String(value).slice(1)) || 0;
  },
  gainItem(data, amount) {
    if (!this._GITHGT) return;
    const next = this.numItems(data) + amount;
    if (next > 0) this._items[data.id] = this.thTyZhNumGain(next);
    else delete this._items[data.id];
  }
};
...
assert.equal(
  encoded._GITHGT,
  false,
  "gate value must remain restored after removal"
);
