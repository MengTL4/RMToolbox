// Bridge harness test: assemble the bridge into a temp file and run it inside a
// vm sandbox with a mock RPG Maker MV game, drive commands through the JSONL
// queue and verify events, state and hook behaviour. No real game process
// involved, and nothing under the repository is written.
...
function makeMockGame(sandbox) {
  ...
  class Game_Actor extends Game_BattlerBase {
    ...
    forgetSkill(id) {
      this._skills = this._skills.filter((s) => s !== id);
    }
    recoverAll() {
      this._hp = 200;
      this._mp = 100;
    ...
    skills() {
      return this._skills.map((id) => ({ id, name: `Skill${id}` }));
    }
    equips() {
      return [];
    }
    ...
    states() {
      return (this._states || []).map((id) => ({ id, name: `State${id}` }));
    }
    addState(id) {
      this._states = (this._states || []).concat(id);
    }
    removeState(id) {
      this._states = (this._states || []).filter((s) => s !== id);
    }
  }
  class Game_Player {
    constructor() {
  ...
  const party = {
    ...
    items() {
      return [
        ...
        { id: 2, name: "Ether", iconIndex: 200 }
      ];
    },
    weapons() {
      return [{ id: 1, name: "Sword", iconIndex: 1 }];
    },
    armors() {
      return [{ id: 1, name: "Shield", iconIndex: 2 }];
    },
    allMembers() {
      return actors;
    },
    ...
    battleMembers() {
      return actors.slice(0, 4);
    },
    addActor(id) {
      actors.push(new Game_Actor(id));
    },
    removeActor(id) {
      /* noop for test */
    },
  ...
  sandbox.$gameActors = {
    actor: (id) => actors.find((a) => a.actorId() === id)
  };
  sandbox.$gameMap = {
    _mapId: 1,
    _displayName: "Field",
  ...
  sandbox.$dataSkills = [
    null,
    { id: 1, name: "Fire", iconIndex: 3 },
    { id: 2, name: "Heal", iconIndex: 4 }
  ];
  sandbox.$dataActors = [
    null,
    { id: 1, name: "Actor1" },
  ...
  sandbox.DataManager = {
    makeSaveContents() {
      return {
        gold: sandbox.$gameParty._gold,
        switches: sandbox.$gameSwitches._data,
        marker: "before"
      };
    },
    extractSaveContents(contents) {
      this._extracted = contents;
    },
    ...
    loadGlobalInfo() {
      ...
      const globalFile = path.join(
        sandbox.process.env.RMCH_GAME_ROOT,
        "www",
        "save",
        "global.rpgsave"
      );
...
async function main() {
  ...
  // Assemble into a temp file rather than the committed
  // runtime/bridge/page-bridge.js: this test used to rewrite a git-tracked file
  // on every run, so a stale parts directory surfaced as spurious working-tree
  // changes. gui-check.mjs still owns the stale-bridge comparison.
  const bridgePath = buildBridge(
    projectRoot,
    path.join(
      mkdtempSync(path.join(tmpdir(), "rmch-bridge-")),
      "page-bridge.js"
    )
  );
  ...
  // 2. gold add/set
  assert.equal(
    (await sendCommand("gold.add", { amount: 500 })).payload.gold,
    500
  );
  assert.equal(
    (await sendCommand("gold.set", { value: 1234 })).payload.gold,
    1234
  );

  // 3. switch / variable / self switch
  assert.equal(
    (await sendCommand("switch.set", { id: 1, value: true })).payload.value,
    true
  );
  assert.equal(mock.switches.value(1), true);
  assert.equal(
    (await sendCommand("variable.set", { id: 2, value: 777 })).payload.value,
    777
  );
  assert.equal(mock.variables.value(2), 777);
  await sendCommand("selfSwitch.set", {
    mapId: 1,
    eventId: 5,
    letter: "A",
    value: true
  ...
  // 4. item add
  const item = await sendCommand("item.add", {
    kind: "item",
    id: 1,
    amount: 9
  });
  ...
  const setMore = await sendCommand("item.set", {
    kind: "item",
    id: 1,
    count: 20
  });
  ...
  const setZero = await sendCommand("item.set", {
    kind: "item",
    id: 1,
    count: 0
  });
  assert.equal(setZero.payload.count, 0);
  const setRestore = await sendCommand("item.set", {
    kind: "item",
    id: 1,
    count: 9
  });
  ...
  assert.equal(
    ether && ether.count,
    3,
    "fractional count must round for display"
  );
  delete mock.party._items[2];
  const realGainItem = mock.party.gainItem;
  mock.party.gainItem = function () {}; // native no-op stub
  const setStub = await sendCommand("item.set", {
    kind: "item",
    id: 1,
    count: 15
  });
  assert.equal(
    setStub.payload.count,
    15,
    "writeback must pin the count past a stub gainItem"
  );
  assert.equal(mock.party._items[1], 15);
  const addStub = await sendCommand("item.add", {
    kind: "item",
    id: 1,
    amount: 2
  });
  assert.equal(
    addStub.payload.count,
    17,
    "item.add past a stub must land exactly"
  );
  ...
  const vitals = await sendCommand("actor.vitals.set", {
    id: 1,
    hp: 199,
    mp: 99
  });
  ...
  assert.equal(
    (await sendCommand("actor.info", { id: 1 })).payload.actor.id,
    1
  );
  ...
  classActor.currentClass = function () {
    return sandbox.$dataClasses[Number(this._classId.split("-")[1])];
  };
  classActor.changeClass = function (id) {
    this._classId = "encoded-" + id;
  };
  assert.equal(
    (await sendCommand("actor.info", { id: 1 })).payload.actor.classId,
    1
  );
  assert.equal(
    (await sendCommand("actor.class.set", { id: 1, classId: 2 })).payload.actor
      .classId,
    2
  );
  for (const invalid of [0, null, -1, 99]) {
    assert.equal(
      (await sendCommand("actor.class.set", { id: 1, classId: invalid })).ok,
      false
    );
    assert.equal(classActor._classId, "encoded-2");
  }
  assert.equal(
    (await sendCommand("actor.class.set", { id: 1, classId: 1 })).payload.actor
      .classId,
    1
  );
  classActor.changeClass = function () {};
  assert.equal(
    (await sendCommand("actor.class.set", { id: 1, classId: 2 })).ok,
    false,
    "native class overrides must not return false success"
  );
  classActor._zwEnsureClass_Multi = function () {};
  const weaponClass = await sendCommand("actor.class.set", {
    id: 1,
    classId: 2
  });
  ...
  delete classActor._classId;
  delete classActor.currentClass;
  delete classActor.changeClass;
  ...
  // 7. catalogs
  const catalog = await sendCommand("catalog.query", {
    kind: "item",
    query: "pot"
  });
  ...
  const options = await sendCommand("trainer.options.set", {
    options: { noSkillCost: true, invincible: false }
  });
  assert.equal(options.payload.options.noSkillCost, true);
  const actor1 = mock.actors[0];
  assert.equal(
    actor1.skillMpCost({ mpCost: 5 }),
    0,
    "noSkillCost must zero skillMpCost for actors"
  );

  // 10. lock HP through the setHp hook
  await sendCommand("trainer.options.set", {
    options: { lockHp: true, lockHpVal: 150, noSkillCost: false }
  });
  actor1.setHp(10);
  assert.equal(
    actor1._hp,
    150,
    "setHp must be forced back to the locked value"
  );
  actor1.addParam = function () {
    this.refresh();
  };
  const ignoredParam = await sendCommand("actor.param.add", {
    id: 1,
    paramId: 2,
    value: 10
  });
  assert.equal(
    ignoredParam.ok,
    false,
    "a game-disabled addParam must not report success"
  );
  delete actor1.addParam;
  const appliedParam = await sendCommand("actor.param.add", {
    id: 1,
    paramId: 2,
    value: 10
  });
  assert.equal(
    appliedParam.ok,
    true,
    "native parameter fallback still applies changes"
  );
  ...
  sandbox.BattleManager.makeRewards = function () {
    this._rewards = { exp: 100, gold: 200 };
  };
  await sendCommand("trainer.options.set", {
    options: { expRate: 2, goldRate: 3, lockHp: false }
  });
  sandbox.BattleManager._phase = "battle";
  const beforeNativeGold = mock.party.gold();
  mock.party.gainGold(10);
  assert.equal(
    mock.party.gold() - beforeNativeGold,
    30,
    "unnamed live party prototype receives the battle gold multiplier"
  );
  ...
  assert.equal(
    sandbox.BattleManager._rewards.gold,
    600,
    "gold rate must apply"
  );
  sandbox.BattleManager._phase = "init";

  // 12. map transfer / through
  const unsafeTransfer = await sendCommand("map.transfer", {
    mapId: 2,
    x: 5,
    y: 6
  });
  assert.equal(
    unsafeTransfer.ok,
    false,
    "transfer outside a map scene must be refused"
  );
  ...
  await sleep(1100);
  assert.equal(
    stairPlayer.isThrough(),
    true,
    "bridge timer must preserve stair-event through state"
  );
  ...
  assert.equal(
    stairPlayer.isThrough(),
    true,
    "trainer through must remain effective after an event disables native through"
  );
  assert.equal(
    stairPlayer._through,
    false,
    "trainer must not write through into game/save state"
  );
  await sendCommand("map.through.set", { value: false });
  assert.equal(
    stairPlayer.isThrough(),
    false,
    "disabling trainer restores native collision immediately"
  );
  stairPlayer.setThrough(true);
  ...
  assert.equal(
    stairPlayer.isThrough(),
    true,
    "toggling trainer must preserve a running event's native through"
  );
  stairPlayer.setThrough(false);
  ...
  assert.equal(
    ce.ok,
    false,
    "whole event execution requires a current map and validated event revision"
  );
  ...
  const itemLock = await sendCommand("lock.set", {
    kind: "item",
    id: 1,
    value: 42
  });
  assert.equal(itemLock.ok, true, itemLock.payload && itemLock.payload.error);
  assert.deepEqual(itemLock.payload, {
    kind: "item",
    id: 1,
    enabled: true,
    value: 42
  ...
  assert.equal(
    mock.party._items[1],
    42,
    "locked item count must be forced back"
  );
  ...
  assert.equal(
    mock.variables._data[1],
    555,
    "locked variable must be forced back"
  );
  ...
  Object.defineProperty(mock.party, "_gold", {
    configurable: true,
    get() {
      return ledgerGold;
    },
    set() {
      throw new Error("raw gold write invalidates ledger");
    }
  });
  mock.party.gainGold = (amount) => {
    ledgerCalls++;
    ledgerGold += amount;
  };
  ...
  assert.equal(
    ledgerGold,
    500,
    "gold lock must preserve the game's gainGold bookkeeping"
  );
  ...
  assert.equal(
    ledgerCalls,
    callsAtTarget,
    "unchanged locks must not repeatedly notify the game"
  );
  await sendCommand("lock.set", { kind: "gold", enabled: false });
  Object.defineProperty(mock.party, "_gold", {
    configurable: true,
  ...
  mock.party.gainGold = (delta) => {
    cappedGoldCalls++;
    mock.party._gold =
      "encoded:" + Math.max(0, Math.min(100, mock.party.gold() + delta));
  };
  const cappedGold = await sendCommand("gold.set", { value: 500 });
  assert.equal(
    cappedGold.payload.gold,
    100,
    "native encoded gold caps remain authoritative"
  );
  ...
  assert.equal(
    cappedGoldCalls,
    callsAtCap,
    "an unreachable gold lock does not repeat native notifications"
  );
  await sendCommand("lock.clear");
  mock.party.gold = oldGoldReader;
  mock.party.gainGold = oldGainGold;
  delete mock.party.maxGold;
  ...
  assert.deepEqual(
    replaced.payload.locks.item,
    { 2: 5 },
    "replace must drop the previous item locks"
  );
  ...
  const plainItems = mock.party._items,
    plainGainItem = mock.party.gainItem;
  mock.party._items = { 1: "encoded:5" };
  mock.party.numItems = (item) => {
    const raw = mock.party._items[item.id];
    if (typeof raw === "number")
      throw new Error("raw inventory write invalidates encoding");
    return raw ? Number(raw.split(":")[1]) : 0;
  };
  mock.party.gainItem = (item, amount) => {
    mock.party._items[item.id] =
      "encoded:" + Math.max(0, mock.party.numItems(item) + amount);
  };
  const encodedSet = await sendCommand("item.set", {
    kind: "item",
    id: 1,
    count: 8
  });
  assert.equal(
    encodedSet.payload.count,
    8,
    "inventory updates read back through numItems"
  );
  assert.equal(
    mock.party._items[1],
    "encoded:8",
    "inventory writes preserve encoded backing fields"
  );
  const encodedAdd = await sendCommand("item.add", {
    kind: "item",
    id: 1,
    amount: 2
  });
  ...
  assert.equal(
    encodedList.payload.entries.find((e) => e.kind === "item" && e.id === 1)
      .count,
    10
  );
  ...
  assert.equal(
    mock.party._items[1],
    "encoded:12",
    "inventory locks use the engine's write path"
  );
  await sendCommand("lock.clear");
  ...
  mock.party.gainItem = () => {};
  const refusedEmptySlot = await sendCommand("item.set", {
    kind: "item",
    id: 1,
    count: 5
  });
  assert.equal(
    refusedEmptySlot.ok,
    false,
    "encoded inventory cannot fall back to writing raw empty slots"
  );
  assert.equal(mock.party._items[1], undefined);
  delete mock.party.thTyZhNumGain;
  delete mock.party.thTyZhNumGet;
  mock.party._items = plainItems;
  mock.party.gainItem = plainGainItem;
  delete mock.party.numItems;
  const oldVariableValue = mock.variables.value,
    oldVariableSet = mock.variables.setValue;
  mock.variables._data[1] = "encoded:4";
  mock.variables.value = (id) =>
    Number(String(mock.variables._data[id]).split(":")[1]) || 0;
  mock.variables.setValue = (id, value) => {
    mock.variables._data[id] = "encoded:" + value;
  };
  ...
  assert.equal(
    mock.variables._data[1],
    "encoded:25",
    "variable locks preserve engine encoding"
  );
  await sendCommand("lock.clear");
  mock.variables.value = oldVariableValue;
  mock.variables.setValue = oldVariableSet;
  mock.variables._data[1] = 555;
  ...
  assert.equal(
    sandbox.DataManager._extracted.marker,
    "after",
    "edited contents must reach extractSaveContents"
  );
  ...
  assert.equal(
    (await sendCommand("save.load", { id: 1 })).ok,
    true,
    "MZ async load resolves zero on success"
  );
  sandbox.DataManager.loadGame = () => false;
  assert.equal(
    (await sendCommand("save.load", { id: 1 })).ok,
    false,
    "MV synchronous false remains failure"
  );
  sandbox.DataManager.loadGame = () =>
    Promise.reject(new Error("native load failed"));
  assert.equal(
    (await sendCommand("save.load", { id: 1 })).ok,
    false,
    "MZ rejection remains failure"
  );
  ...
  assert.equal(
    sceneInfo.payload.current,
    "Scene_Map",
    "published native constructor identity survives obfuscated class names"
  );
  assert.ok(
    sceneInfo.payload.available.includes("Scene_Item"),
    "Scene_Item must be offered"
  );
  assert.ok(
    !sceneInfo.payload.available.includes("Scene_Equip"),
    "absent scenes must be filtered out"
  );
  ...
  const idleRouteRepair = await sendCommand("game.repair", {
    action: "clearMoveRoute"
  });
  assert.equal(
    idleRouteRepair.ok,
    true,
    "clearing an absent move route must succeed"
  );
  ...
  assert.equal(
    sandbox.$gamePlayer._routeEnded,
    true,
    "existing routes retain native cleanup"
  );
  ...
  const nick = await sendCommand("actor.nickname.set", {
    id: 1,
    nickname: "Tester"
  });
  ...
  const withoutState = await sendCommand("actor.state.remove", {
    id: 1,
    stateId: 3
  });
  ...
  assert.equal(
    battleInfo.ok,
    true,
    battleInfo.error || "battle.info must not throw"
  );
  assert.equal(battleInfo.payload.inBattle, true);
  assert.equal(battleInfo.payload.enemies.length, 2);
  assert.equal(
    battleInfo.payload.enemies[0].name,
    "Slime",
    "name() must be preferred"
  );
  assert.equal(
    battleInfo.payload.enemies[1].name,
    "Bat",
    "$dataEnemies must be the fallback"
  );
  ...
  deathStateEnemy.die = function () {
    this._hp = 0;
    this._states = [];
  };
  deathStateEnemy.deathStateId = function () {
    return 1;
  };
  deathStateEnemy.addState = function (id) {
    this.die();
    this._states.push(id);
  };
  deathStateEnemy.isAlive = function () {
    return this._states.indexOf(1) < 0;
  };
  ...
  guardedDeathEnemy.die = function () {
    this._states = [];
  };
  guardedDeathEnemy.deathStateId = function () {
    return 1;
  };
  guardedDeathEnemy.addState = function () {};
  guardedDeathEnemy.addNewState = function (id) {
    this.die();
    this._states.push(id);
  };
  guardedDeathEnemy.isAlive = function () {
    return this._states.indexOf(1) < 0;
  };
  ...
  assert.equal(
    guardedDeathEnemy._hp,
    0,
    "custom die() may only update states; forced defeat must also clear native HP"
  );
  sandbox.BattleManager._phase = "init";
  ...
  try {
    assert.equal(
      actor.skillMpCost(),
      5,
      "suppressNoCost must restore the real cost"
    );
  } finally {
    bridgeObject.suppressNoCost -= 1;
  }
  assert.equal(
    actor.skillMpCost(),
    0,
    "cost is waived again once the scope exits"
  );
  ...
  assert.ok(
    mockWindow.showCount > 0,
    "watchdog must show a never-visible window"
  );
  ...
  await sleep(1700);
  assert.equal(
    mockWindow.showCount,
    shownBeforeDisarm,
    "watchdog must disarm once the window is visible"
  );
  ...
  while (
    !sandbox.DataManager.loadGlobalInfo.__rmchPatched &&
    Date.now() < guardWait
  )
    await sleep(100);
  assert.ok(
    sandbox.DataManager.loadGlobalInfo.__rmchPatched,
    "guard must patch loadGlobalInfo"
  );
  const guardedInfo = sandbox.DataManager.loadGlobalInfo();
  assert.deepEqual(
    guardedInfo,
    [],
    "throwing loadGlobalInfo must degrade to []"
  );
  assert.ok(!existsSync(globalFile), "corrupt global file must be quarantined");
  const quarantined = readdirSync(mockSaveDir).filter((name) =>
    name.startsWith("global.rpgsave.corrupt-")
  );
  assert.equal(
    quarantined.length,
    1,
    "quarantine must keep a renamed backup, not delete"
  );
  assert.equal(
    sandbox.DataManager._globalCalls,
    2,
    "guard must retry the original after quarantining"
  );
  const infoAgain = sandbox.DataManager.loadGlobalInfo();
  assert.deepEqual(
    infoAgain,
    [],
    "missing-file path must now serve [] without drama"
  );
  ...
  assert.equal(
    sealedParty.payload.gold,
    null,
    "without a capture the party is gone"
  );
  const sealedGold = await sendCommand("gold.set", { value: 100 });
  assert.equal(
    sealedGold.ok,
    false,
    "gold.set must fail while nothing resolves"
  );
  const sealedSwitches = await sendCommand("switch.list", {});
  assert.equal(
    sealedSwitches.ok,
    false,
    "switch.list must fail while $dataSystem and the store are both gone"
  );
  ...
  assert.ok(
    sandbox.__rmchCapture,
    "tap must capture saveContents on stringify"
  );
  assert.equal(
    sandbox.__rmchCapture.party,
    mock.party,
    "capture must hold the live party object"
  );
  ...
  assert.equal(
    capturedGold.ok,
    true,
    `gold.set via capture failed: ${JSON.stringify(capturedGold)}`
  );
  assert.equal(
    mock.party._gold,
    4321,
    "capture-backed party must be the live object"
  );
  ...
  assert.equal(
    swList34.ok,
    true,
    `switch.list via capture failed: ${JSON.stringify(swList34)}`
  );
  ...
  assert.equal(
    (await sendCommand("switch.set", { id: 2, value: true })).payload.value,
    true
  );
  assert.equal(
    mock.switches.value(2),
    true,
    "capture-backed switches must be the live store"
  );
  assert.equal(
    (await sendCommand("variable.set", { id: 3, value: 555 })).payload.value,
    555
  );
  ...
  const potion34 = inv34.payload.entries.find(
    (e) => e.kind === "item" && e.id === 1
  );
  assert.equal(
    potion34 && potion34.name,
    "Potion",
    "owned item name comes from party.items()"
  );
  assert.equal(potion34 && potion34.count, mock.party._items[1]);
  assert.equal(
    (await sendCommand("item.set", { kind: "item", id: 1, count: 30 })).payload
      .count,
    30
  );
  assert.equal(mock.party._items[1], 30);
  const unowned = await sendCommand("item.set", {
    kind: "item",
    id: 99,
    count: 1
  });
  assert.equal(
    unowned.ok,
    false,
    "unowned item without a catalog must fail cleanly"
  );
  ...
  assert.equal(
    sandbox.__rmchCapture.party,
    replacementParty,
    "capture must refresh on the next save"
  );
  ...
  const throwingParty = {
    ...
    gainGold() {
      throw new Error("Cannot read property 'constructor' of null");
    },
    gainItem() {},
    allMembers() {
      return [];
  ...
  assert.equal(
    (await sendCommand("gold.add", { amount: 50 })).payload.gold,
    150,
    "gold.add must fall back to a direct write when gainGold throws"
  );
  assert.equal(throwingParty._gold, 150);
  assert.equal(
    (await sendCommand("gold.set", { value: 200 })).payload.gold,
    200,
  ...
  assert.equal(
    (await sendCommand("gold.add", { amount: 25 })).payload.gold,
    125,
  ...
  vm.runInContext(
    "JSON.parse(window.__db.items); JSON.parse(window.__db.system); JSON.parse(window.__db.maps);",
    context
  );
  ...
  vm.runInContext(
    'JSON.parse(\'[{"wtypeId":1,"params":[]},{"wtypeId":2,"params":[]}]\')',
    context
  );
  assert.ok(
    !sandbox.__rmchDataTables.weapon,
    "0-based array must not classify"
  );
  vm.runInContext(
    'JSON.parse(\'[null,{"wtypeId":1,"params":[]},{"itypeId":1,"price":3}]\')',
    context
  );
  assert.ok(
    !sandbox.__rmchDataTables.weapon,
    "mixed-shape array must not classify"
  );

  // Catalog/system-name/map queries fall back to the captured tables (the
  // window globals were deleted in group 34).
  const cat35 = await sendCommand("catalog.query", {
    kind: "item",
    limit: 20000
  });
  assert.equal(
    cat35.payload.total,
    2,
    `captured item catalog failed: ${JSON.stringify(cat35)}`
  );
  const sw35 = await sendCommand("switch.list", {});
  assert.equal(
    sw35.payload.namesUnavailable,
    false,
    "captured $dataSystem must restore names"
  );
  ...
  assert.equal(
    map35.payload.total,
    2,
    `captured mapInfo catalog failed: ${JSON.stringify(map35)}`
  );
  ...
  vm.runInContext(
    'JSON.parse(\'[null,{"id":1,"name":"重甲","atypeId":1,"etypeId":2,"price":30}]\')',
    context
  );
  await sleep(1500);
  const cache35b = JSON.parse(readFileSync(cachePath35, "utf8"));
  assert.equal(
    cache35b.tables.weapon[1].name,
    "OldBlade",
    "flush must keep disk-only kinds"
  );
  assert.equal(
    cache35b.tables.armor[1].name,
    "重甲",
    "flush must add newly captured kinds"
  );
  assert.equal(
    cache35b.tables.item[1].name,
    "Elixir",
    "flush must keep in-memory kinds"
  );
  ...
  vm.runInContext(
    "delete window.__rmchDataTables; delete window.__rmchDataTablesLoaded;",
    context
  );
  const cat35b = await sendCommand("catalog.query", {
    kind: "item",
    limit: 20000
  });
  assert.equal(
    cat35b.payload.total,
    2,
    "catalog must come back from the disk cache"
  );
  assert.ok(
    sandbox.__rmchDataTables,
    "lazy load must repopulate __rmchDataTables"
  );
  ...
  assert.equal(
    sandbox.__rmchCapture.party._gold,
    999,
  ...
  assert.equal(
    Object.keys(sandbox2.__rmchDataTables).length,
    2,
    "junk hole-array must not classify"
  );
  assert.equal(
    sandbox2.__rmchCapture && sandbox2.__rmchCapture.party._gold,
    5,
  ...
  assert.ok(
    existsSync(cachePath36),
    "replayed tables must flush to catalog-cache.json"
  );
