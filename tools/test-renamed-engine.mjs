...
const source = readFileSync(
  new URL("../runtime/bridge/src/parts/07-renamed-engine.js", import.meta.url),
  "utf8"
);
const resolveSource = readFileSync(
  new URL("../runtime/bridge/src/parts/10-engine.js", import.meta.url),
  "utf8"
);
const sandbox = {
  ThGem_Player: class Player {},
  ThSce_Map: class MapScene {},
  ThGem_Interpreter: class Interpreter {},
  ThGem_Actor: class Actor {},
  ThWin_Menu: class Menu {},
  ThBtData: {},
  $thNewGmPl: null,
  $thNewGmPr: null,
  $thNewGmMp: null,
  $thNewGmAc: null,
  $thNewGmMe: null,
  $thNewDtAc: [null, { id: 1, name: "driver" }],
  $thNewDtMpIf: [null, { id: 1, name: "town" }],
  SceneManager: {
    thNewGoto(scene) {
      this.next = scene;
    },
...
assert.equal(
  sandbox.SceneManager.next,
  sandbox.ThSce_Map,
  "scene transitions preserve manager receiver"
);
...
assert.equal(
  vm.runInContext("resolveParty()", context),
  second,
  "load/new-game must replace the live reference"
);
sandbox.$gameParty = first;
assert.equal(sandbox.$thNewGmPr, first, "write-through preserves game globals");
assert.equal(
  vm.runInContext('resolveData("actor")', context),
  sandbox.$thNewDtAc
);
assert.equal(
  vm.runInContext('resolveData("mapInfo")', context),
  sandbox.$thNewDtMpIf
);
...
assert.equal(
  Object.getOwnPropertyDescriptor(sandbox, "$gameParty").get,
  descriptor.get,
...
assert.equal(
  unrelated.Game_Player,
  undefined,
  "one matching constructor is not a family signature"
);
const storm = {
  Game_Player: class {},
  Scene_Map: class {},
  ...
  $ftGmSw: {},
  $ftGmVr: {},
  $newTkIt: [null, { id: 1, name: "capsule" }],
  $newTkAc: [],
  $newTkCs: [],
  $thTkSk: [],
  $thTkWp: [],
...
assert.equal(
  vm.runInContext("resolveParty()", stormContext),
  first,
  "FT family publishes its real party"
);
...
console.log(
  "renamed engine PASS: family gate, native-global preservation, constructors, resolvers and live save replacement"
);
