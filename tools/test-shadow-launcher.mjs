...
function main() {
  ...
  try {
    ...
    vm.runInNewContext(patchBundledEngineScript(bundledSource), {
      window: bundledWindow
    });
    for (const name of ["Scene_Item", "Scene_Battle", "Game_Action"])
      assert.equal(
        bundledWindow[name],
        bundledWindow.expected[name],
        name + " must be published from the native bundle closure"
      );
    ...
    const { appDir, gameExe, bgScriptPath } = setupShadowApp({
      projectRoot,
      scan,
      gameKey: "fake-game"
    });
    ...
    assert.ok(
      ...
      "shadow package.json must exist"
    );

    // Big directories are junctioned wholesale…
    assert.ok(
      lstatSync(path.join(appDir, "www")).isSymbolicLink(),
      "www must be a junction"
    );
    assert.ok(
      lstatSync(path.join(appDir, "locales")).isSymbolicLink(),
      "locales must be a junction"
    );
    ...
    assert.ok(
      shadowBgDir.isDirectory() && !shadowBgDir.isSymbolicLink(),
      "bg_script in the shadow must be a real directory, not a junction"
    );
    assert.ok(
      existsSync(path.join(appDir, "bg_script", "helper.js")),
    ...
    assert.ok(
      patched.includes("ORIGINAL BG SCRIPT"),
      "patched bg-script must embed the original"
    );
    assert.ok(
      patched.includes("process.cwd"),
      "patched bg-script must carry the prelude spoof"
    );
    assert.ok(
      patched.includes("page-bridge"),
      "patched bg-script must carry the bridge suffix"
    );
    const nativeModules = [
    ...
    const shadowProcess = {
      env: {},
      cwd: () => gameRoot,
      report: { getReport: () => ({ sharedObjects: [...nativeModules] }) }
    };
    vm.runInNewContext(patched, {
      require: (name) => (name === "fs" ? fs : path),
      process: shadowProcess,
      location: {
        href: "chrome-extension://test/_generated_background_page.html"
      }
    });
    assert.deepEqual(
      shadowProcess.report.getReport().sharedObjects,
      nativeModules.slice(2),
      "shadow bootstrap filters only the observed font compatibility modules"
    );
    ...
    assert.equal(
      readFileSync(path.join(gameRoot, bgScript), "utf8"),
      originalBg,
    ...
    assert.equal(
      existsSync(path.join(appDir, "wmic.exe")),
      false,
      "plain shadow must not retain a previous guarded launch shim"
    );
    ...
    makeFile(
      path.join(gameRoot2, "package.json"),
      JSON.stringify({
        name: "fake-game-2",
        main: "www/index.html",
        "bg-script": "loading"
    ...
    const { bgScriptPath: patchedPath2 } = setupShadowApp({
      projectRoot,
      scan: scan2,
      gameKey: "fake-game-2"
    });
    assert.ok(
      readFileSync(patchedPath2, "utf8").includes("ORIGINAL LOADER"),
      "root-level bg-script must be patched in the shadow"
    );
    assert.equal(
      readFileSync(path.join(gameRoot2, "loading"), "utf8"),
      "// ORIGINAL LOADER\n",
    ...
    vm.runInNewContext(readFileSync(named.bgScriptPath, "utf8"), {
      require: (name) => (name === "fs" ? fs : path),
    ...
    assert.equal(
      namedProcess.execPath,
      namedExe,
      "shadow prelude must preserve the actual source executable name"
    );
    ...
    makeFile(
      path.join(projectRoot, "runtime", "bin", "wmic.exe"),
      "fake-wmic-shim"
    );
    const gameRoot3 = path.join(tempRoot, "game3");
    const stockManifest = JSON.stringify({
      name: "fake-game-3",
    ...
    const { appDir: appDir3, bgScriptPath: patchedPath3 } = setupShadowApp({
      projectRoot,
      scan: scan3,
      gameKey: "fake-game-3"
    });
    assert.equal(
      readFileSync(path.join(appDir3, "package.json"), "utf8"),
      stockManifest,
      "grover shadow manifest copy must stay byte-identical to the game's"
    );
    assert.ok(
      existsSync(path.join(appDir3, "wmic.exe")),
      "grover shadow must carry the wmic shim"
    );
    const patched3 = readFileSync(patchedPath3, "utf8");
    assert.ok(
      patched3.includes("ORIGINAL GROVER LOADER"),
      "grover patched bg-script must embed the original"
    );
    assert.ok(
      patched3.includes("__rmchGroverGuards"),
      "grover patched bg-script must carry the guards bootstrap"
    );
    assert.ok(
      patched3.includes("crashRenderer"),
      "grover guards must cover crashRenderer"
    );
    assert.equal(
      readFileSync(path.join(gameRoot3, "loading"), "utf8"),
      "// ORIGINAL GROVER LOADER\n",
      ...
      for (const layout of ["root", "www"]) {
        for (const nested of [false, true]) {
          ...
          const script = nested
            ...
            : "engine.js";
          const source =
            '(function () { var originalMarker = "ORIGINAL"; }.call(this));';
          const manifest = JSON.stringify({
            name: key,
            main: layout === "www" ? "www/index.html" : "index.html"
          });
          ...
          makeFile(
            path.join(root, path.dirname(script), "sibling.dat"),
            "sibling"
          );
          ...
          for (const [file, seconds] of [
            [path.join(realSave, "file1.rmmzsave"), 200],
            [path.join(oldSave, "file1.rmmzsave"), 100],
            [path.join(realSave, "file2.rmmzsave"), 100],
            [path.join(oldSave, "file2.rmmzsave"), 200]
          ])
          ...
          if (nested) {
            ...
            symlinkSync(
              path.join(root, path.dirname(script)),
              parent,
              "junction"
            );
          } else {
          ...
          const scan = {
            root,
            layout,
            manifest: { bgScript: script },
            bundled: { scriptRel: script }
          };
          ...
          assert.equal(
            ...
            source,
            `${key}: original patch source unchanged`
          );
          assert.equal(
            readFileSync(path.join(root, "package.json"), "utf8"),
            manifest,
            `${key}: original manifest unchanged`
          );
          assert.ok(readFileSync(patchPath, "utf8").includes("ORIGINAL"));
          assert.notEqual(readFileSync(patchPath, "utf8"), source);
          assert.equal(
            readFileSync(path.join(realSave, "file1.rmmzsave"), "utf8"),
            "REAL-NEWER"
          );
          assert.equal(
            readFileSync(path.join(realSave, "file2.rmmzsave"), "utf8"),
            "SHADOW-NEWER"
          );
          assert.equal(
            readFileSync(path.join(realSave, "file3.rmmzsave"), "utf8"),
            "RESCUED"
          );
          writeFileSync(
            path.join(app, saveRel, "file4.rmmzsave"),
            "WRITE-THROUGH"
          );
          assert.equal(
            readFileSync(path.join(realSave, "file4.rmmzsave"), "utf8"),
            "WRITE-THROUGH"
          );
          ...
          assert.equal(
            readFileSync(path.join(root, "package.json"), "utf8"),
            manifest,
            `${key}: manifest copy is independent`
          );
          ...
          assert.equal(
            ...
            firstPatch,
            `${key}: rebuild does not accumulate patches`
          );
          assert.equal(
            readFileSync(path.join(realSave, "file4.rmmzsave"), "utf8"),
            "WRITE-THROUGH",
            `${key}: rebuilding existing links preserves real saves`
          );
        }
      }
    }
    for (const kind of ["bg", "bundled"]) {
      ...
      const app = path.join(projectRoot, "runtime", "shadow-apps", key);
      const source =
        '(function () { var marker = "SHARED ORIGINAL"; }.call(this));';
      ...
      assert.equal(
        readFileSync(path.join(shared, "js", "engine.js"), "utf8"),
        source,
        "source junction never receives patch writes"
      );
      assert.equal(
        readFileSync(path.join(shared, "save", "file1.rmmzsave"), "utf8"),
        "real-save"
      );
    }
    ...
    assert.throws(
      () =>
        setupBundledShadowApp({
          projectRoot,
          gameKey: "invalid",
          scan: { root: invalidRoot, bundled: { scriptRel: "engine.js" } }
        }),
      /anchor not found/
    );
    assert.equal(
      existsSync(path.join(projectRoot, "runtime", "shadow-apps", "invalid")),
      false,
      "bad patch fails before filesystem rebuild"
    );
    console.log(
