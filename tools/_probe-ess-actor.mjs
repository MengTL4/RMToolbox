...
await check("actor.recover 1", async () => {
  ...
  if (p.actor.hp !== p.actor.mhp)
    throw new Error(`hp ${p.actor.hp}/${p.actor.mhp}`);
  return `hp=${p.actor.hp}/${p.actor.mhp}`;
});

const before = await session
  .send("actor.info", { id: 1 }, 30000)
  .then((p) => p.actor)
  .catch(() => null);
if (before) {
  ...
  await check(`actor.level.set back ${before.level}`, async () => {
    const p = await session.send(
      "actor.level.set",
      { id: 1, level: before.level },
      30000
    );
    if (p.actor.level !== before.level)
      throw new Error(`level=${p.actor.level}`);
  });
}
...
await check("actor.state.add BURN", async () => {
  ...
  if (!(p.actor.states || []).some((s) => s.id === "BURN"))
    throw new Error(JSON.stringify(p.actor.states));
...
await check("actor.skill.forget CONFUSION", async () => {
  const p = await session.send(
    "actor.skill.forget",
    { id: 1, skillId: "CONFUSION" },
    30000
  );
  if ((p.actor.skills || []).some((m) => m.id === "CONFUSION"))
    throw new Error("still knows CONFUSION");
});
await check("actor.skill.learn TACKLE", async () => {
  ...
  if (!(p.actor.skills || []).some((m) => m.id === "TACKLE"))
    throw new Error(JSON.stringify(p.actor.skills));
...
await check("actor.skill.forget TACKLE", async () => {
  ...
  if ((p.actor.skills || []).some((m) => m.id === "TACKLE"))
    throw new Error("still knows TACKLE");
});
