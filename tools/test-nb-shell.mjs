...
function makeFakeNbEvalNwBinGame(root) {
  mkdirSync(path.join(root, "nb_data"), { recursive: true });
  writeFileSync(path.join(root, "nb_data", "8bc38d774d3c0a3c2b67177c512ccffa.node"), "MZ fake hashed addon");
  writeFileSync(path.join(root, "nb_data", "981e939328ec49d3af7b7a01b4e106a4"), "fake v8 bytecode blob");
  writeFileSync(path.join(root, "nb_data", "011dc36ebfd76d8d5c34c5b95617b91a.json"), "xzuOQt3KmTVxcfBk"); // ciphertext
  writeFileSync(path.join(root, "index.html"), "<!doctype html><html><body><script>var _0xodz='nobi';/* obfuscated */</script></body></html>");
  writeFileSync(
    path.join(root, "package.json"),
    JSON.stringify({ name: "1", main: "index.html", window: { title: "万族穿越【测试】" }, "chromium-args": "--disable-devtools" })
  );
...
async function main() {
  ...
  try {
    ...
    assert.equal(evalScan.container, "nb-evalnwbin", "hashed .node + hashed blob must label nb-evalnwbin");
    assert.equal(evalScan.engine.id, "MV/MZ", "engine is undecidable from the outside");
    ...
    writeFileSync(path.join(bothRoot, "nb_data", "nbtool.node"), "MZ fake nbtool node addon");
    assert.notEqual(scanGame(bothRoot).container, "nb-evalnwbin", "nbtool.node must not fall to evalnwbin");

    // launchGame must refuse the (fatal) extension launch path; attach with no
    // running game must fail BEFORE injecting anything.
    try {
      await launchGame({ gameRoot: evalRoot, projectRoot: tempRoot, port: 47417 });
    ...
    try {
      await attachGame({ gameRoot: evalRoot, projectRoot: tempRoot, port: 47417 });
      assert.fail("attachGame without a running game must fail");
    } catch (error) {
      assert.ok(error instanceof AttachError, "attach failure is an AttachError");
